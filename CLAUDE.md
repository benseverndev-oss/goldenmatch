...
- A `ci.yml` edit forces the full matrix (`force_all`) **only in the merge queue**, NOT at PR time — a lane you expected to prove your change can silently skip.
- Adding a new job means a new filter entry AND the `if:` gate. **Reusing a neighbouring job's filter means the job never runs on the code it executes** — `dead_code`/`goldenmatch_sweep_coverage` were gated on `python_goldenmatch` while their source lives in `scripts/`, so a detector-only PR went green without running the detector. `scripts/check_filter_coverage.py` now ratchets this.
