...
- A `ci.yml` edit forces the full matrix (`force_all`) **only in the merge queue**, NOT at PR time — see ci.yml's `force_all` comment. At PR time `workflow_lint` covers the YAML cheaply and the merge queue is where the filter wiring is re-validated against main. Do not assume touching `ci.yml` re-runs every lane on a PR; it does not, and a lane you expected to prove your change can silently skip.
- Adding a new job means adding a new filter entry AND wiring the `if:` gate. **Reusing a neighbouring job's filter is how a job ends up never running on changes to the code it executes** — the `dead_code` and `goldenmatch_sweep_coverage` jobs were gated on `python_goldenmatch` (which watches `packages/python/goldenmatch/**`) while their source lives in `scripts/`, so a detector-only PR went green having never run the detector. `scripts/check_filter_coverage.py` now ratchets against this.
