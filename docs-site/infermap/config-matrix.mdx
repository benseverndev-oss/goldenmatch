...
4 MCP tool(s) exposed by `infermap.mcp.server` -- the programmatic / agent surface. Each accepts a JSON input; the config-bearing tools (`auto_configure`, `dedupe`, `match`, ...) take the same knobs as above.

`apply`, `inspect`, `map`, `validate`
