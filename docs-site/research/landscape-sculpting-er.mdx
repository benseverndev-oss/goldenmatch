...
basins (recall), impure basins get a ridge raised to split them (precision), iterating until every marble settles in a clean basin that represents an entity.
...
accuracy on solved benchmarks; the bottleneck isn't the clustering algorithm*, directly motivates the next direction.
