...
| `key.integrity` | A GoldenMatch `KeyIntegrityCertificate` (`key_certificate`) | Key uniqueness-at-grain, duplicate-key groups, max fan-out, and: when the certificate's resolution tier ran, the fragmented-entity count and undercount estimate, plus a `measure_fan_out` table. The read-side of the semantic-layer key-integrity wedge (produced by `goldenmatch.semantic.certify_key_integrity`). Python-only in v1. |
