...
(`goldenflow-duckdb`) — a compiled Rust extension that links the same reference
kernels straight into DuckDB, so the transforms run natively in the query engine,
byte-identical to the Python / TypeScript / WASM surfaces. 74 transforms are
...
Download the per-platform zip from the `goldenflow-duckdb-v*`
