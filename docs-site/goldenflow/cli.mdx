...
| `GOLDENFLOW_NATIVE` | Native-kernel path (reference-mode): `auto`/unset uses native where a kernel exists (phone + checksummed identifiers), `0` forces pure-Python, `1` requires native. See [Performance](/goldenflow/performance). |
