...
| `goldencheck mcp-serve` | Start the MCP server (19 tools). stdio by default; `--transport http` refuses to bind a non-loopback host (the default `0.0.0.0`) unless `GOLDENCHECK_MCP_TOKEN` is set (clients then send `Authorization: Bearer <token>`), or `GOLDENCHECK_MCP_ALLOW_PUBLIC=1` deliberately opens it. Binding `--host 127.0.0.1` needs neither. |
