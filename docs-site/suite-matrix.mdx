...
**21 of 24 scorers are kernel-backed** (the reference fast path); the other 3 are pure-language fallbacks with no `-core` kernel.

| Substrate | Scorers |
|---|---|
| Rust `-core` kernel — Python + TS | `alias_match`, `array_intersect`, `audio_fp`, `date`, `date_diff`, `dice`, `ensemble`, `exact`, `geo_haversine`, `given_name_aliased_jw`, `initialism_match`, `jaccard`, `jaro_winkler`, `levenshtein`, `name_freq_weighted_jw`, `numeric_diff`, `phash`, `qgram`, `radial`, `soundex_match`, `token_sort` |
| Rust `-core` kernel — Python arrow-native only (TS falls back) | — |
| WASM `-core` kernel — TS only (Python falls back) | — |
| Language fallback — no `-core` kernel | `cosine`, `embedding`, `record_embedding` |
...
| `goldenanalysis` | analyzers | 4 | 0 | 0 |
