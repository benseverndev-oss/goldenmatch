...
**17 of 20 scorers are kernel-backed** (the reference fast path); the other 3 are pure-language fallbacks with no `-core` kernel.
...
| Language fallback — no `-core` kernel | `date_diff`, `embedding`, `record_embedding` |
...
- `goldenmatch` **scorers** — Python-only: `date_diff`.
