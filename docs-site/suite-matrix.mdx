...
**20 of 23 scorers are kernel-backed** (the reference fast path); the other 3 are pure-language fallbacks with no `-core` kernel.
...
| Rust `-core` kernel — Python arrow-native only (TS falls back) | `array_intersect` |
