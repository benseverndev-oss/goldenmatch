...
| Substrate | Scorers |
|---|---|
| Rust `-core` kernel — Python + TS | `alias_match`, `array_intersect`, `audio_fp`, `date`, `date_diff`, `dice`, `ensemble`, `exact`, `geo_haversine`, `given_name_aliased_jw`, `initialism_match`, `jaccard`, `jaro_winkler`, `levenshtein`, `name_freq_weighted_jw`, `phash`, `qgram`, `radial`, `soundex_match`, `token_sort` |
...
- `goldenmatch` **scorers** — Python-only: `numeric_diff`.
