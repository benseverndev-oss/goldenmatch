...
|---|---|---|---|---|
| `goldenmatch` | mcp_tools | 67 | 15 | 8 |
| `goldenmatch` | cli_commands | 13 | 23 | 1 |
| `goldenmatch` | a2a_skills | 32 | 15 | 10 |
...
- `goldenmatch` **mcp_tools** — Python-only: `create_domain`, `documents_ingest`, `documents_suggest_schema`, `explain_routing`, `identity_profile`, `identity_show`, `identity_stats`, `identity_worklist`, `lint_routing`, `list_domains`, `list_plugins`, `plan_routing`, `pprl_auto_config`, `pprl_link`, `test_domain`.
...
- `goldenmatch` **a2a_skills** — Python-only: `analyze_blocking`, `compare_clusters`, `configure`, `documents_ingest`, `documents_suggest_schema`, `identity_show`, `incremental`, `list_runs`, `pprl`, `quality`, `retrieve_similar`, `review`, `rollback`, `schema_match`, `sensitivity`.
