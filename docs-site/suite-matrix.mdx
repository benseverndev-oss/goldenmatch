...
**22 of 24 scorers are kernel-backed** (the reference fast path); the other 2 are pure-language fallbacks with no `-core` kernel.

| Substrate | Scorers |
|---|---|
| Rust `-core` kernel — Python + TS | `alias_match`, `array_intersect`, `audio_fp`, `cosine`, `date`, `date_diff`, `dice`, `ensemble`, `exact`, `geo_haversine`, `given_name_aliased_jw`, `initialism_match`, `jaccard`, `jaro_winkler`, `levenshtein`, `name_freq_weighted_jw`, `numeric_diff`, `phash`, `qgram`, `radial`, `soundex_match`, `token_sort` |
...
| `infermap` | mcp_tools | 5 | 0 | 0 |
| `infermap` | cli_commands | 6 | 0 | 0 |
