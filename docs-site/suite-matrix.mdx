...
**17 of 22 scorers are kernel-backed** (the reference fast path); the other 5 are pure-language fallbacks with no `-core` kernel.
...
| Language fallback — no `-core` kernel | `date_diff`, `embedding`, `geo_haversine`, `numeric_diff`, `record_embedding` |
