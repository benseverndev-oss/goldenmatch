...
| `GOLDENMATCH_FRAME` | `polars` | `polars`/`arrow` | Frame backend for file ingest. `arrow` is EXPERIMENTAL: file ingest via pyarrow, part of the Polars-eviction program. Output-equivalence enforced by a differential harness + frozen fixtures. |
