...
| `GOLDENMATCH_FS_BLOCK_SOURCE` | `auto` | Select the FS streaming block source: `sequential` (in-RAM Arrow/Rust batches — the fused FS kernel groups + scores + clusters in one call, no spill), `duckdb` (the DuckDB file-spilled out-of-core path, the past-RAM tier), `frame`/`eager` (in-memory bucket variants), or `auto` (pick by dataset). Unifies the older `GOLDENMATCH_FS_OUT_OF_CORE=1` toggle (now an alias for `duckdb`). |
