...
| `GOLDENMATCH_TOKEN_BLOCKING` | `0` (off) | Let auto-suggest propose the `token` blocking strategy (DF-pruned inverted-index token blocking) for free-text columns. The strategy itself is always available from an explicit config; this flag only controls whether blocking auto-suggest may *choose* it. | Leave off for now. The strategy reaches 98.2% blocking recall on Amazon-Google against the committed key's 7.15%, but committing it automatically took that benchmark's end-to-end F1 from 0.1014 to **0.0000**: the failing auto-config subprofile flips to `blocking`, `build_token_blocks` never runs, and the committed RED config yields no candidate pairs. Undiagnosed (#2488), so default off until the benchmark says otherwise. |
