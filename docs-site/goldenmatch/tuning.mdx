...
| `GOLDENMATCH_FS_CALIBRATE_THRESHOLD` | `1` (on) | Pick the FS link cutoff per-dataset from the training-pair score distribution (Otsu's between-class-variance split) instead of the fixed `0.50` default. Adapts the cutoff to each dataset's over/under-merge: raises it where namesakes pile up just below (historical_50k F1 +0.04, precision 0.72->0.93), lowers it where the data under-merges (dblp_acm F1 +0.49, recall 0.27->0.93), and leaves well-separated data unchanged (febrl3/synthetic). Default off is byte-identical. |
