...
| `GOLDENMATCH_FS_TF_CLAMP` | unset (10 bits) | Measurement knob for `GOLDENMATCH_FS_TF_ADJUSTMENT`: overrides the +/- bit clamp on the Winkler term-frequency bump `log2(collision / freq(value))`. TF under the default calibration regresses historical_50k (-0.0125) and cotenant_hardneg (-0.0209) while inflating predicted pairs, and the inflation shrinks as the evidence bar rises; lowering the clamp tests whether the size of the bump is the cause. **Any value, 10 included, routes TF fields off the native kernel**, which applies its own 10-bit clamp and cannot see this override -- so `=10` is a same-clamp control for native/numpy parity. Empty, non-numeric or non-positive values mean unset. Default unset is byte-identical. |
