...
| `GOLDENMATCH_FS_ORTHOGONAL_BLOCKING` | `0` (off) | Field-agnostic orthogonal-anchor blocking. Adds an additive single-field blocking pass on every well-populated, moderate-cardinality field the name/date/zip passes don't already cover, so an orthogonal identity anchor a dataset's classifier missed (e.g. `birth_place` profiling as `name`) still co-locates the pairs its corrupted name keys can't. Additive passes stay EM-trained (co-locate without demoting the discriminator). A PERSON/PII-regime lever (historical_50k F1 +0.021, febrl4 holdout +0.006) that REGRESSES on bibliographic data (dblp_scholar −0.24, where the "anchor" is a topic bucket) — hence default off, byte-identical. |
