...
| `GOLDENMATCH_FS_COLUMNAR_CLUSTER` | `1` (on) | B2c (#1811, **default-on #2006**): thread the Arrow pair stream STRAIGHT to the columnar cluster path (`build_clusters_columnar` over `_columnar_pairs_df`) on the eligible single-FS-matchkey in-memory bucket dedupe, so the driver-resident `all_pairs` Python `list[tuple]` is never built during scoring → clustering — the accumulator that runs to hundreds of millions of tuples before clustering starts at 14M (the late-stage OOM of #1811). The in-memory analogue of `GOLDENMATCH_FS_OOC_ARROW_CLUSTER`; superset of `GOLDENMATCH_FS_ARROW_STREAM` (also drops `matched_pairs`). The review band rides the Arrow table and is still attached to `review_pairs` (bounded by the review cut, as on the list path). Clusters are NOT byte-identical (the FS bucket pipeline is ~0.1%-nondeterministic run-to-run regardless), so the flip moves the default output within that run-to-run band only; ineligible configs (multi-matchkey, across-files, semantic blocking, bench-dump, non-bucket scale backends) fall back to the list path unchanged. Set `0`/`false` to force the legacy list path (the byte-parity oracle). The post-cluster `scored_pairs` list is deduped columnar + shed above `GOLDENMATCH_FS_SCORED_PAIRS_MAX` (#2006). |
