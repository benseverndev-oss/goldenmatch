...
| `GOLDENMATCH_ENSEMBLE_KERNEL` | unset (off — matrix path) | Routes the `ensemble` scorer through a per-pair bucket kernel instead of declining to the float32 matrix. `1`/`on`/`f32` = the field-exact variant, byte-identical to the matrix (`f32(max(a,b,c)) == max(f32(a),f32(b),f32(c))`), so zero recall/precision change; `f64` = a measurement-only A/B twin that diverges ≤3e-8. Default (unset) is unchanged behavior. | Opt-in only, leave unset. The per-pair path is Python-per-pair, so it can be slower than the vectorized matrix on very large `ensemble` blocks — the reason it's not default-on. See `docs/superpowers/specs/2026-07-21-ensemble-kernel-measurement.md`. |
