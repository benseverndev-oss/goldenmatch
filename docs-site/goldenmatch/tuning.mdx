...
| `GOLDENMATCH_FS_LINEAR_CUT` | unset (off) | Place the **linear** FS link cutoff by evidence bits instead of at the fixed 0.50. The linear score normalizes `W` against the summed per-field weight extremes, so 0.50 links at the midpoint of those extremes, which moves whenever one field's top weight does (ncvr_synthetic at a 50,000-pair u sample: +7.4 → −1.9 bits). `prior` links at `W >= max(log2((1 - λ) / λ), 0)`, where the posterior crosses 0.5; `prior_mid` uses the larger of that and the old midpoint. Converted to the equivalent normalized cutoff, so every scorer applies it; an explicit `link_threshold` or calibrated cutoff still wins, and posterior mode is untouched. Measurement lever. |
