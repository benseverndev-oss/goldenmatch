...
| `GOLDENMATCH_DISTRIBUTED_ON_ERROR` | `raise` | Worker failure policy for the distributed scoring path. `raise` (default) fails the run when a partition cannot score, its standardization fails, or a blocking-key build fails -- all of which otherwise contribute FEWER matches while the run reports success, and none of which Ray can retry once the exception is swallowed. `skip` restores the old lossy behaviour and logs at ERROR; the dropped count is not aggregated to the driver, so treat it as an escape hatch rather than a mode. |
...
| `GOLDENMATCH_IDENTITY_SQLITE_BATCH` | `1` (on) | Wrap the SQLite identity resolve writes in explicit transactions instead of per-statement autocommit. The SQLite connection is opened `isolation_level=None`, so before this every INSERT paid a WAL sync on its own (~6 statements per cluster). Measured on a 200k-row store: ~750 µs/statement autocommit vs ~30–90 µs inside a transaction (8–25x). SQLite-only; no-op on Postgres/Mongo. | `0` restores the per-statement autocommit path, for parity debugging. |
| `GOLDENMATCH_IDENTITY_SQLITE_BATCH_SIZE` | `10000` | Statements per SQLite transaction inside `bulk_writes` (when batching is on). The batch commits and re-opens every N writes so the WAL cannot grow without bound on a multi-million-row resolve: the memory-bounded half of the fix. Non-positive or unparseable values fall back to `10000`. | Lower to cap WAL growth harder; raise to commit less often. |
| `GOLDENMATCH_IDENTITY_BULK` | `1` (on) | Route brand-new clusters through the bulk write fast-path in `resolve_clusters` (staging table + `executemany` + `INSERT..SELECT..ON CONFLICT`) instead of the per-cluster row loop, on both SQL backends (Postgres COPY / SQLite staging). Cut a 40k-singleton SQLite `emit_singletons=True` resolve ~47× (185.9s → 3.95s). Store contents are byte-identical to the per-row path. | `0` restores the per-row `upsert_*` loop, for parity debugging. |
| `GOLDENMATCH_IDENTITY_BULK_FLUSH_ROWS` | `250000` | Source-record rows accumulated before the bulk fast-path flushes a batch. Caps the accumulator (and per-flush frame) at O(batch) so the write side never stacks a second O(N) frame-residency term on the prep. Small/medium resolves stay a single flush (unchanged behaviour). Non-positive or unparseable values fall back to `250000`. | Lower to bound write-side memory harder on very large resolves; raise to flush less often. |
| `GOLDENMATCH_IDENTITY_PREP_CHUNK_ROWS` | `250000` | Rows per chunk in `apply_batch`'s prep pass. The prep used to run three whole-frame passes (`select_dicts` over every row, a 5M-entry `h1_by_rowid`, then the derivation loop); stage-level RSS at 5M attributes ~3.9 GB to the row-dict list and ~0.6 GB to `h1_by_rowid`, of the 10.4 GB `apply_batch` adds. Streaming fuses them into one chunked pass, so the row-dict working set is O(chunk) not O(N) and each chunk's hashes are consumed in place. Derived values are unchanged -- chunks are contiguous slices in original order and `batch_fingerprints` is per-row -- verified against the single pass across uneven chunk sizes. | `0` restores the single-pass form, for parity debugging. |
| `GOLDENMATCH_IDENTITY_INITIAL_LOAD` | `0` (off) | Opt-in initial-load fast path for a from-empty Postgres build (`IdentityStore.initial_load_writes`). Drops the secondary indexes, routes the bulk writes to a direct COPY into the real tables (no temp staging, no `INSERT..SELECT`, no `ON CONFLICT`), then rebuilds the indexes and `ANALYZE`s. Sound only from empty tables of brand-new clusters, so the store gates on emptiness and it is a no-op on a non-empty store. Postgres-only. | `1` enables it; leave `0` for incremental re-resolves. |
| `GOLDENMATCH_IDENTITY_INITIAL_LOAD_UNLOGGED` | `0` (off) | Under the initial-load fast path, also load the tables `UNLOGGED` (skip WAL) and `SET LOGGED` after. Only safe when the build is re-runnable from source, since an `UNLOGGED` table is truncated on crash and `SET LOGGED` rewrites it. Ignored unless `GOLDENMATCH_IDENTITY_INITIAL_LOAD` is on and engages. | `1` enables the WAL skip; leave `0` for a durable load. |
...
| `GOLDENMATCH_AUTOCONFIG_DETERMINISTIC` | `0` (off) | Removes every wall-clock cut from the auto-config search: the controller's `max_seconds` stop and the per-indicator time budgets, leaving the iteration caps as the only stopping rule. With it off, how far the search gets (and so which config is committed) depends on how fast and how loaded the host is, which makes any pin or gate over that config unstable: red on a slow runner with no code change, green on a fast one while masking a real change. Runtime becomes unbounded in exchange. | Set `1` alongside `GOLDENMATCH_AUTOCONFIG_MEMORY=0` for anything that pins, blesses or gates auto-config output. Leave off for interactive runs, which want a bounded wait more than a bit-reproducible search. |
...
| `GOLDENMATCH_TOKEN_BLOCKING` | `1` (on) | Let auto-suggest propose the `token` blocking strategy (DF-pruned inverted-index token blocking) for free-text columns. The strategy itself is always available from an explicit config; this flag only controls whether blocking auto-suggest may *choose* it. | Leave on. Only columns that measure as free text are eligible, so names and addresses keep their prefix/soundex keys; on Amazon-Google the token plan reaches 0.982 blocking recall against the previously committed key's 0.041. It shipped default-OFF under #2488 on the reading that committing it took that benchmark's F1 to **0.0000** -- that was wrong: nothing ever set `strategy="token"`, so `build_token_blocks` never ran and the 0.0000 came from whatever config was committed instead (#2717). Set to `0` to force auto-suggest back off. |
...
| `GOLDENMATCH_AUTOCONFIG_DETERMINISTIC` | `0` | `0`/`1` | Drop the search's wall-clock budgets so the committed config is host-independent. `1` for anything that pins or gates auto-config output. |
...
| `GOLDENMATCH_KEY_INTEGRITY_NATIVE` | off | `0`/`1` | Route the structural key-integrity certifier (`certify_key_integrity`) through the shared `key-integrity-core` Rust kernel instead of the pyarrow group_by. Opt-in and fail-open (a missing native symbol / non-JSON-safe group column falls back to pyarrow). The pyarrow path stays the default + reference: the kernel path exists for the single-Rust-owner guarantee (shared with the TS/WASM surface), not speed (it is JSON-marshaled). |
...
| `GOLDENMATCH_IDENTITY_SQLITE_BATCH` | `1` (on) | Batch the SQLite identity resolve writes into explicit transactions (#2105). `0` = per-statement autocommit. |
| `GOLDENMATCH_IDENTITY_SQLITE_BATCH_SIZE` | `10000` | Statements per SQLite transaction inside `bulk_writes`; bounds WAL growth on large resolves. |
| `GOLDENMATCH_IDENTITY_BULK` | `1` (on) | Bulk write fast-path for brand-new clusters in `resolve_clusters` (Postgres COPY / SQLite staging). `0` = per-row loop. |
| `GOLDENMATCH_IDENTITY_BULK_FLUSH_ROWS` | `250000` | Records accumulated before the bulk fast-path flushes; bounds write-side memory to O(batch). |
| `GOLDENMATCH_IDENTITY_PREP_CHUNK_ROWS` | `250000` | Rows per chunk in `apply_batch`'s streamed prep; bounds the row-dict working set to O(chunk). `0` = single pass. |
...
| `GOLDENMATCH_BLOCKING_DIVERSIFY_PAIR_BUDGET` | `1` (on) | Hold the orthogonal-anchor passes that `_diversify_probabilistic_blocking` adds (date YEAR, postcode/zip/identifier) to the same `GOLDENMATCH_BLOCKING_PAIRS_PER_ROW` budget the primary-key selector uses. Previously those passes were bounded only by the FS scorer's per-block ROW cap, which limits per-block memory and not the WORK a pass creates: measured on person @ 100K, the birth-YEAR anchor sits at 774 pairs/row against a budget of 50 and accounts for 71,337,394 of 121,391,850 comparisons (59%) while costing **zero** F1 to remove, whereas the postcode anchor is 4 pairs/row and carries all the recall (-0.0269 F1 without it). Both clear the row cap, so it cannot tell them apart. Enabling takes the plan to 50,054,456 comparisons at an unchanged pairwise F1 0.9970. `0` restores the row-cap-only behaviour -- the measurement is person-shaped, and the lever exists for error-heavy PII where a cheap-looking anchor may be the only thing co-blocking corrupted names. |
...
| `GOLDENMATCH_BLOCKING_COST_AWARE` | `0` | Demote a small-fixed-domain year/date column from the sole primary blocking key when a bounded name/compound fallback exists (it stays a recall pass), so identifier-poor data doesn't explode candidate pairs at scale. Domain-routed: skipped on bibliographic data, where a publication-year key is a legitimately strong same-year blocking signal. **Stays opt-in:** the 2026-08-07 OFF-vs-ON bench measured it as a byte-identical no-op on both bench shapes (the bench person fixture has a `postcode`, so a year key never becomes the sole primary there), so there is no measured evidence for a default flip: enable it only on genuinely identifier-poor data. |
...
| `GOLDENMATCH_FS_BUCKET_BATCH` | `1` (on) | Coalesce a bucket's small blocks into row-capped (`GOLDENMATCH_FS_BATCH_ROWS`) batched vectorized-scorer calls on the **non-native** bucket path (when the native kernel is declined, e.g. `missing="disagree"`), instead of one `prob_scorer` call per block. Byte-identical clusters/partitions. `0` = per-block loop (parity escape hatch). Only affects the vectorized-numpy scorer; scalar/ensemble/embedding fallbacks stay per-block. |
...
| `GOLDENMATCH_FS_OVERSIZED_CHUNK_PAIRS` | `4000000` | Candidate-pair budget for ONE scoring call on an **un-splittable oversized** block (`skip_oversized=false` and auto-split found no useful split: the #1826 hub shape). Above it the block is scored in bounded **tiles** instead of whole, so a single call never carries the block's full `C(n,2)`: the vectorized lane's dense matrix stays ~`16 × budget` bytes per field (~64 MB at the default) instead of `O(n²)`, and the native lane stops double-buffering the whole block's surviving pairs. Same pair set and scores, ~2× the pair comparisons **on that block only** (cross tiles recompute the intra-group pairs they discard); emission order within the block becomes tile order. `0` = score whole (pre-#2417 parity hatch). See #1826/#2417. |
...
| `GOLDENMATCH_FS_TF_CLAMP` | unset (10 bits) | Measurement knob for `GOLDENMATCH_FS_TF_ADJUSTMENT`: overrides the +/- bit clamp on the Winkler term-frequency bump `log2(collision / freq(value))`. TF under the default calibration regresses historical_50k (-0.0125) and cotenant_hardneg (-0.0209) while inflating predicted pairs, and the inflation shrinks as the evidence bar rises; lowering the clamp tests whether the size of the bump is the cause. **Any value, 10 included, routes TF fields off the native kernel**, which applies its own 10-bit clamp and cannot see this override -- so `=10` is a same-clamp control for native/numpy parity. Measured under the default calibration: historical_50k -0.0116 / -0.0043 / +0.0058 at clamp 10 / 6 / 3, while cotenant_hardneg (-0.0178) and household_hardneg (-0.0041) are unchanged at every clamp, so TF still fails the gate (ADR 0066). Empty, non-numeric or non-positive values mean unset. Default unset is byte-identical. |
| `GOLDENMATCH_FS_FIELD_DEPENDENCE` | `0` (off) | Correct Fellegi-Sunter's conditional-independence assumption on correlated field PAIRS. FS sums per-field weights as if each field's agreement were independent given match status; where two fields co-agree more often than chance among NON-matches, the true joint `u` exceeds `u_a*u_b`, so FS under-counts the denominator and over-weights every pair agreeing on both -- the namesake double-count. EM estimates the excess-lift bits `log2(u_joint / (u_a_top*u_b_top))` over the posterior-weighted non-match population and scoring subtracts them when both fields agree at top level. Measured on historical_50k (run 34358803727): `first_name x surname` co-agree **5.57x** independence, **+2.48 bits** over-counted on **97%** of false merges. Until #2919 it never reached scoring on the default multi-pass bucket route (the EM combine dropped it and the native bucket kernel bypassed it), so every panel reading between #2914 and #2919 was of inert code (the July 2026 spike predated that route, did reach scoring, and found the same precision-for-recall trade). Measured once running, with each arm at its own best `GOLDENMATCH_FS_EVIDENCE_CUT`: ties on person and ncvr_synthetic, loses on historical_50k, household_hardneg and cotenant_hardneg (ADR 0066). A single fixed cut cannot judge it -- the correction shrinks the evidence scale, so the same bar under-links the ON arm. Enabling it forces the numpy scoring path. Default off is byte-identical (no correction estimated). Since ADR 0066's addendum it subtracts the net two-class double-count by default (see `GOLDENMATCH_FS_FD_CLASSES`), which is at least as good as the original at every bar on every panel dataset but still does not beat the uncorrected baseline. |
| `GOLDENMATCH_FS_FD_CLASSES` | unset (`both`) | Which EM class(es) `GOLDENMATCH_FS_FIELD_DEPENDENCE` measures. The default subtracts the NET double-count, `log2(u_ab/(u_a*u_b)) - log2(m_ab/(m_a*m_b))`: non-match lift minus match lift. Loglinear record-linkage models include interactions in either or both classes, and dependence matters most in the DOMINATING class (Xu et al. 2019); inside historical_50k's blocks match prevalence is ~0.92. Measured with each arm at its own best `GOLDENMATCH_FS_EVIDENCE_CUT`: at least as good as the non-match-only correction on every panel dataset at every bar, better on historical_50k (0.8124 vs 0.7812), still below the uncorrected baseline (ADR 0066 addendum). `nonmatch` restores the original. |
| `GOLDENMATCH_FS_SIGNATURE_PRUNE` | unset (off) | Measurement lever: prune multi-pass FS candidates by WHICH blocking passes a pair co-fires in, before EM trains and before the link/review split, so the model trains on the population it scores. Passes whose co-firing phi over the candidates is at least 0.2 merge into one group, so correlated name passes count once. `1` / `on` / `dominant`: drop a pair that fires only one group when that group fires on at least 30% of candidates -- the namesake-sized group. `multi`: keep only pairs that fire two or more groups. Label-free, measured as pair completeness before any F1 run: household_hardneg / cotenant_hardneg keep 51.7% / 33.0% of candidates under `dominant` and 4.5% / 4.9% under `multi`, at pair completeness 1.0 either way; on historical_50k with an added `surname` soundex pass, `dominant` keeps 48.7% at pair completeness 0.9193 (today: 0.8793 over all 11.98M), while `multi` loses 18% of reachable pairs. An EM mixture over the same groups was rejected: its rare-match mode is not stable. **Measured end to end, it stays off.** With each arm at its own best `GOLDENMATCH_FS_EVIDENCE_CUT` (default, 3, 5, 9, 12), `dominant` gains **+0.0063 F1 on historical_50k** (0.8320 -> 0.8383) and ties on every other panel dataset. That gain flips to -0.0159 at cut 9, and nearly all of it comes from EM's retrained sample, not from dropping scored pairs. `multi` loses 0.0120 on historical_50k (recall). With a forced `surname` soundex pass, the pass alone costs 0.0276 and pruning recovers at most 0.0019, so it does not rescue that pass. household_hardneg's first_name soundex group fires on 0.30 of candidates, on the threshold itself, and swings between -0.0027 and -0.0288. The pipeline's pass signatures compare block KEYS and ignore oversized-block splitting, so on historical_50k they see 27.2M candidates against the 11.98M actually built. Dedupe scope on the bucket routes only; elsewhere it logs DECLINED rather than filtering EM alone. Disables `GOLDENMATCH_FS_EM_COUNTED` and never writes the filtered model to `model_path`. Default off is byte-identical. Harness: `scripts/bench_er_headtohead/measure_blocking_signatures.py`. |
| `GOLDENMATCH_FS_FD_PROBE` | unset (off) | Diagnostic for `GOLDENMATCH_FS_FIELD_DEPENDENCE`: log, per corrected field pair, how many pairs the correction actually touches, where their evidence sits relative to the bar, and how many fall in the flippable band `[cut, cut+bits)` -- the only pairs a subtraction can reclassify. Exists because the correction measured `dF1 +0.0000` on all five panel datasets while provably firing at up to **-7.62 bits**: it fires only where BOTH fields agree at their TOP level, and such a pair is an overwhelming match sitting far above the bar, so the subtraction may land exclusively on pairs whose decision was never in doubt. Requires `GOLDENMATCH_FS_EVIDENCE_CUT` (without a prior-invariant bar there is no distance to measure) and says so rather than guessing. Its weights cover NON-conditioned fields only, understating the engine's by roughly +0.93 b per conditioned field -- biased toward reporting MORE near-cut pairs than exist, so an empty band is the stronger result. Never changes the correction it measures. |
...
| `GOLDENMATCH_FS_ORTHOGONAL_BLOCKING` | `auto` | Field-agnostic orthogonal-anchor blocking. Adds an additive single-field blocking pass on every well-populated, moderate-cardinality field the name/date/zip passes don't already cover, so an orthogonal identity anchor a dataset's classifier missed (e.g. `birth_place` profiling as `name`) still co-locates the pairs its corrupted name keys can't. Additive passes stay EM-trained (co-locate without demoting the discriminator). `auto` (default) fires ONLY on person-shaped data (a `name` + a `date` col_type): the regime where it generalises (historical_50k B³ 0.808→0.873, febrl4 holdout +0.006), and is a no-op on bibliographic/product data (dblp_scholar −0.24, no name+date). `1` forces everywhere; `0` disables. |
| `GOLDENMATCH_FS_ATOMIC_NAME_BLOCKING` | `off` | Atomic-name-soundex blocking completion. Adds an additive `surname`/`first_name` soundex pass when names are blocked via a composite soundex pass but no atomic single-name pass exists (a corrupted first name otherwise breaks the composite key). **Known-negative: leave off.** It raises candidate/blocking recall +6pp (historical_50k 0.886→0.946), but the canonical `bench_er_headtohead` panel measures it as a *regression*: historical_50k pairwise F1 −0.0148 (B³ −0.0078), because the corrupted-name candidates fall below the FS threshold and the EM shift degrades the operating point (both precision and recall drop). febrl3/febrl4 no-op. Kept gated for reference. `auto` = person-shaped only; `1`/`on` = everywhere, do NOT enable except to experiment. |
...
| `GOLDENMATCH_FS_CALIBRATE_THRESHOLD` | `0` (off) | Pick the FS link cutoff per-dataset from the training-pair score distribution (Otsu's between-class-variance split) instead of the fixed `0.50` default. Adapts the cutoff to each dataset's over/under-merge: raises it where namesakes pile up just below (historical_50k F1 +0.04, precision 0.72->0.93), lowers it where the data under-merges (dblp_acm F1 +0.49, recall 0.27->0.93), and leaves well-separated data unchanged (febrl3/synthetic). Default off is byte-identical. |
| `GOLDENMATCH_FS_REFIT_THRESHOLD` | `0` (off) | Refit the FS link cutoff from the ACTUAL scored-pair distribution (re-cluster only, no re-scoring): the health-gated threshold-refit loop. Finds the class-separating VALLEY between a false-pair band and the true-match mass, but commits it only behind two guards: a deep-valley gate (the trough must be near-empty, not a shoulder) and a cluster-shape guard (accept only if re-clustering there reduces the max cluster size). Recovers over-merge the fixed `0.50` can't see on over-merge-prone shapes (household hard-negatives F1 0.947→1.000, co-tenant address over-merge 0.318→0.996). **Was flipped ON 2026-08-02, reverted to OFF 2026-08-07 (#2387)**, the cluster-shape guard has a known defect: `max_candidate < max_default` is a single-outlier statistic, so it accepts a candidate that fixes one over-merged cluster while shattering many correct ones. On `ncvr_synthetic` (all gold clusters exactly size 2) it commits `0.50→0.70`, satisfies its own objective (max cluster 5→2), and still costs **−0.0966 F1**, which reddened the nightly `bench-suggest-quality` gate for five nights. Enable with `=1` (or `true`/`on`/`yes`/`enabled`) if your data's dominant error mode is over-merge; default off is byte-identical to the fixed cutoff. Re-flipping needs a global accept criterion + a panel including ncvr_synthetic. Spec: `docs/superpowers/specs/2026-08-02-fs-refit-loop-design.md`. |
...
| `GOLDENMATCH_FS_BLOCK_SOURCE` | `auto` | Select the FS streaming block source: `sequential` (in-RAM Arrow/Rust batches: the fused FS kernel groups + scores + clusters in one call, no spill), `spill` (DuckDB-**free** out-of-core: per-pass scored-edge shards on disk + an external union-find that streams them back, bounds the edge term with no DuckDB dependency), `bucketed` (DuckDB-**free** frame-residency out-of-core: hash-bucket the prepared frame to disk by each blocking pass's key, then score bucket-by-bucket, never a whole-frame scoring copy, so it bounds the FRAME term the `spill`/fused paths leave resident; partition-exact with `sequential`), `duckdb` (the DuckDB file-spilled out-of-core path, the past-RAM tier), `frame`/`eager` (in-memory bucket variants), or `auto` (pick by dataset). Unifies the older `GOLDENMATCH_FS_OUT_OF_CORE=1` toggle (now an alias for `duckdb`). |
| `GOLDENMATCH_FS_OOC_WAVE_ROWS` | `2000000` | Max resident block-rows scored per parallel wave in the in-RAM `sequential` FS scorer (the fallback path when the fused kernel is uncovered): bounds peak to one wave of buffered blocks + their pair results, not a whole blocking pass. `0`/invalid → the 2M default. Output-invariant. |
| `GOLDENMATCH_FS_BUCKET_SCORE_WORKERS` | `min(cpu, 8)` | Thread concurrency for the `bucketed` block source's shard-scoring loop. Shards are independent and `score_buckets_arrow` releases the GIL (native/rapidfuzz), so scoring them concurrently fills the cores/RSS the serial loop left idle during the wall-dominant scoring phase (measured ~−19% wall at 4M on a 4-core box). `1` = serial (byte-identical to the pre-parallel path). Output-invariant: the external union-find is edge/shard-order-invariant. More workers ⇒ more shards in flight ⇒ higher peak RSS, so raise it only with headroom. |
| `GOLDENMATCH_FS_SPILL_FORCE_SHARD` | `0` (off) | Within the `spill` block source, force the edge-shard mechanism (per-pass shards on disk + `external_wcc_from_shards`) by SKIPPING the fused-kernel short-circuit that otherwise clusters a fused-covered config in one call. Default off means `spill` tries the fused kernel first (already bounded, no pair list) and the shard path is a fallback for uncovered configs; `1` reaches the bounded-edge path directly (for benchmarking/validation). Byte-identical partition either way. |
...
| `GOLDENMATCH_FS_COLUMNAR_CLUSTER` | `1` (on) | B2c (#1811/#2006, **default-on, Arrow-native**): thread the `score_buckets_arrow` pair **table** STRAIGHT into the frames-out clustering (`build_cluster_frames` + the native arrow kernel) on the eligible single-FS-matchkey in-memory bucket dedupe: Arrow end-to-end with **no polars**. Neither the driver-resident `all_pairs` Python `list[tuple]` NOR `build_clusters_columnar`'s legacy per-cluster / 16M-key `pair_scores` dict is ever built (the late-stage OOM + the measured ~40s clustering cost at 5M of #1811; scoring itself was already native-fast). The review band rides the Arrow table and still attaches to `review_pairs`. Superset of `GOLDENMATCH_FS_ARROW_STREAM` (also drops `matched_pairs`). Ineligible configs (multi-matchkey, across-files, semantic blocking, bench-dump, non-bucket scale backends) fall back to the list path unchanged. Clusters are NOT byte-identical (the FS bucket pipeline is ~0.1%-nondeterministic run-to-run regardless), so the flip moves the default output within that band only. Set `0`/`false` to force the legacy list path (the byte-parity oracle). Post-cluster `scored_pairs` is deduped Arrow-native + shed above `GOLDENMATCH_FS_SCORED_PAIRS_MAX` (#2006). |
...
| `GOLDENMATCH_FS_POST_BLOCKING_U` | `0` (off) | Opt-in u-probability bias correction for **bibliographic** dedup: FS estimates `u` (non-match agreement) from random pairs but only scores blocked pairs, where blocking-correlated fields agree far more often: inflating a candidate-common field's weight and driving false merges. ON deconvolves `u` from the blocked population in the final weights (`u_blocked = (blocked_rate - p_match*m) / (1 - p_match)`), deflating candidate-common fields toward weight ~0 while keeping discriminative fields. Verified NOT a default (deflates discriminative person-data fields → controller quality gate regresses); default off is byte-identical. |
| `GOLDENMATCH_FS_U_PAIRS` | `min(5 × n_sample_pairs, 50000)` (50,000 at the default) | Random-pair budget for the FS `u` estimate (was `min(n_sample_pairs, 5000)`); setting it replaces the rule outright. Applies to blocked EM training (the pipeline path), counted EM and label-anchored estimation; unblocked `train_em` trains m on the same random pairs and keeps the old sample. `u` is a level's frequency among random, overwhelmingly non-matching pairs; a level those pairs almost never reach (exact title/author agreement on bibliographic data) counts 0 in 5,000 pairs, so its `u` fell to the smoothing floor and its agreement weight exploded past the exact level's. Measured on the FS panel, best F1 per dataset over five cutoffs: dblp_acm 0.8613 → 0.9077, dblp_scholar 0.4213 → 0.5050, no dataset down more than historical_50k's 0.0047. 200,000 cost historical more; 1,000,000 was no better and ~70 s slower per dataset (the sample is scored pair by pair). `5000` restores the previous budget; `0` means the default. |
| `GOLDENMATCH_FS_LINEAR_CUT` | `prior_mid` | Places the **linear** FS link cutoff by evidence bits instead of at the fixed 0.50, and sets the process-wide DEFAULT rule -- a per-matchkey `link_cut_rule` overrides it. Accepts any rule name: `prior`, `prior_mid`, `posterior_099`, `evidence_3`, `evidence_5`, `evidence_9`, `evidence_12`, `otsu` (Otsu's between-class-variance split of the trained model's own training-pair score distribution, same split `GOLDENMATCH_FS_CALIBRATE_THRESHOLD` uses); `midpoint` stays an alias for `off` at this env var rather than a distinct rule. The linear score normalizes `W` against the summed per-field weight extremes, so 0.50 links at the midpoint of those extremes, which moves whenever one field's top weight does (ncvr_synthetic at a 50,000-pair u sample: +7.4 → −1.9 bits). `prior` links at `W >= max(log2((1 - λ) / λ), 0)`, where the posterior crosses 0.5; `prior_mid` uses the larger of that and the old midpoint. Converted to the equivalent normalized cutoff, so every scorer applies it; an explicit `link_threshold` or calibrated cutoff still wins, and posterior mode is untouched. Measured on the FS panel with the 50,000-pair u sample: ncvr_synthetic 0.9743 → 0.9990, dblp_acm 0.8058 → 0.8159, amazon_google 0.0217 → 0.0475, the other seven datasets' clusters unchanged. `off` restores the midpoint cut. The cutoff reports its source as `evidence_rule`. |
| `GOLDENMATCH_FS_CUT_ROUTER` | `off` | Routes the **linear** FS link-cut rule per probabilistic matchkey from its trained model: `fs_cut_rules.choose_cut_rule` reads the cut diagnostics (λ, the midpoint and prior cutoffs in bits, the field count, per-field weight spans) and the first matching row of the shipped table picks the rule; no match keeps the `GOLDENMATCH_FS_LINEAR_CUT` default. `on`/`1`/`true` enables it. A pinned `link_cut_rule`, an explicit `link_threshold`, a calibrated cutoff and posterior mode all win over it. A routed cut reports source `evidence_rule` and `cut_reason` `routed by <row>: <reason>`. A row ships only after never scoring below the default (within 0.01) on every design and held-out dataset of the link-cut matrix (`scripts/autoconfig_quality/cut_gate.py`). Shipped rows, first match wins: `sparse_prior_binds` (λ < 0.004, prior cutoff above the midpoint, at most 3 fields → `posterior_099`); `midpoint_binds_wide` (λ < 0.1, midpoint cutoff above the prior, at least 5 fields → `evidence_5`). An unrecognised value warns and stays off. |
| `GOLDENMATCH_FS_EVIDENCE_CUT` | unset (off) | Opt-in prior- and endpoint-invariant link threshold (bits): set the cut to `sigma(c + prior_w)` on the posterior axis, exactly `W >= c` where `W = sum log2(m/u)`. Decouples the operating point from the min-max envelope so a weight change (TF/honorific/u-correction) no longer relocates every pair's decision. Implies posterior scoring; `GOLDENMATCH_FS_CALIBRATED=linear` overrides. No single `c` fits every dataset (two unsupervised selectors were refuted), so it stays a knob, default OFF (byte-identical). |
...
| `GOLDENMATCH_CLUSTER_SPLIT_TIES` | `level` | The oversized-cluster auto-split cuts every weakest spanning-tree edge that ties, not just the first in endpoint order. Single-linkage at that height, so the split cannot depend on which tied edge sorts first. A component whose edges all tie still loses one edge, so a block of exact duplicates is not shattered. Measured on the FS panel at five cutoffs: never lower, precision up where tied oversized clusters exist (historical_50k 0.8274 → 0.8386, dblp_scholar 0.3757 → 0.4213). `single` restores the one-edge cut. Applies to both Python splitters and the native kernel (`mst_split_components_level`, goldenmatch-native ≥ 0.2.2; older wheels fall back to the pure-Python level cut); TypeScript `buildClusters` takes the same rule as `splitTies: "level" \| "single"`. |
...
| `GOLDENMATCH_SPARK_JAR` | unset | Path to the JVM scorer jar shipped to Spark executors via `addArtifact`. Unset means "look for the build output"; the jar is built rather than vendored, so a source checkout has none until you build it. |
| `GOLDENMATCH_SPARK_VECTOR_SCORER` | `0` | Score every comparison field of a pair in ONE UDF call instead of one per field. **MEASURED SLOWER -- do not enable.** On a 1M-row / 5.5M-pair cluster run it was 2.3x slower than the per-field path (scoring 26.11s -> 59.75s, 17.13s -> 40.38s; counts stage 45.42s -> 101.37s) with byte-identical pattern counts and model. Kept, off, as a recorded negative result: Spark arrays are `ArrayData` of `InternalRow`, so a returned similarity vector pays row-wise object churn -- and `getItem(i)` per field appears not to be common-subexpression-eliminated, so the UDF is likely evaluated once per field anyway, plus the array. Batching inside Spark SQL loses to a columnar transfer. |
| `GOLDENMATCH_SPARK_FUSED_BLOCK_JOIN` | `1` | Score and count INSIDE the blocking self-join rather than projecting it to `(a, b)` ids and joining both records back. `0` forces the legacy three-join path. MEASURED on the plan: one counts pass goes from 7 exchanges / 3 sort-merge joins to 3 / 1, and the score path from 8 / 4 to 4 / 2 -- the joins that disappear are the ones consuming a frame with O(pairs) rows. On the 100M lane this cut shuffle write 43% (174.9 -> 99.0 GB), memory spill 92% (56.4 -> 4.29 GB) and stages 119 -> 74, with identical pairs, scores and quality. **It did NOT move total wall** (958.31s -> 975.30s): on that workload resource use and wall are close to decoupled, and the flag exists so the A/B stays takeable in both directions. |
| `GOLDENMATCH_SPARK_EXACT_PROFILING` | `0` | Measure auto-config column statistics on the CLUSTER instead of on a 20,000-row driver sample. Fixes a statistic that is currently mislabelled: `profile_columns` stores its confirm pass as `full_n_distinct`, documented as "the EXACT full-frame count, never the sampled", but on the Spark path it is handed a driver sample, so on a 500M-row table the "exact" count describes 20k rows. Two rules read it as though it described the population, and both have already cost a run: an `email` column with a true distinct-fraction of 0.28 read 0.72 -> 1.00 across 100K -> 5M and was discarded as a surrogate key (#876), and the drop-constant rule misses a value at frequency 1e-4 90% of the time from a 1,000-row sample (#2687). The verdict moves with scale rather than with the data. Cost is bounded: `approx_count_distinct` runs for every column and an exact `count_distinct` (a shuffle per column) only for boundary columns near `<= 1` or near-unique — cheap statistic nominates, exact one decides. **Default OFF on purpose**: several rules compare these fields to absolute cuts chosen while the inputs were sample-derived, so making them truthful can move quality scores the way rebasing `mass_above_threshold` moved `anchor_person_match` from 1.0000 to 0.7303. This flips on quality-gate evidence, not on the argument that it is more correct. The `exact_profiling` argument overrides the environment in either direction, so a caller or test can pin it without touching process state. |
...
| `GOLDENMATCH_SEMANTIC_NAMER` | unset | Hard kill-switch for the OPTIONAL advisory LLM namer on semantic-model discovery (`discover_semantic_model(name=True)` / `discover-model --name`). The namer is opt-in per call and never authoritative (business names land only in `ProposedModel.naming`; the certified structure is untouched). `0` forces it off even when requested; otherwise it runs when `name=True` and an LLM provider/key resolves (`goldenmatch[llm]`), abstaining to no names otherwise. |
| `GOLDENMATCH_LOCAL_LLM_PATH` | unset | Explicit path to a local GGUF for `provider="local"`, bypassing the pinned-model download. |
| `GOLDENMATCH_LOCAL_LLM_CACHE` | `~/.cache/goldenmatch/models` | Directory the pinned GGUF is downloaded into (from its GitHub Release asset) and cached across runs. |
