...
| `GOLDENMATCH_FS_REFIT_THRESHOLD` | `1` (**on**) | Refit the FS link cutoff from the ACTUAL scored-pair distribution (re-cluster only, no re-scoring) — the health-gated threshold-refit loop. Finds the class-separating VALLEY between a false-pair band and the true-match mass, but commits it only behind two guards: a deep-valley gate (the trough must be near-empty, not a shoulder) and a cluster-shape guard (accept only if re-clustering there reduces the max cluster size, i.e. actually breaks over-merge). Corrects the over-merge the fixed `0.50` can't see on over-merge-prone shapes (household hard-negatives F1 0.947→1.000, co-tenant address over-merge 0.318→0.996) while staying a no-op on 0.50-optimal data. **Default ON since 2026-08-02** — validated: structural no-op on clean data (the cluster-shape guard) + MEASURED flat & perf-neutral at scale (QIS 50k/100k ΔF1 +0.00000, wall within noise), no panel regression. `GOLDENMATCH_FS_REFIT_THRESHOLD=0` (or `false`/`off`/`no`/`disabled`) restores the fixed cutoff. Spec: `docs/superpowers/specs/2026-08-02-fs-refit-loop-design.md`. |
