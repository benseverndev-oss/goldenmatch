...
| `GOLDENMATCH_ENSEMBLE_KERNEL` | `1` (on) | The `ensemble` scorer rides the bucket fast path (per-pair + the float64 `_score_block_vec` vec lane) instead of declining the whole matchkey to the float32 `find_fuzzy_matches` matrix. Measured **1.47× faster** end-to-end on Febrl3 with **byte-identical** recall (TP/FP/FN unchanged). `0` (`off`/`false`) is the kill-switch → the historical decline path. | Leave on. Set `0` only to (a) reproduce pre-2026-07-21 output byte-for-byte, or (b) if a workload has huge *all-ensemble* blocks where the native-accelerated float32 matrix beats the vec lane (rare; `skip_oversized` bounds block size). See `docs/superpowers/specs/2026-07-21-ensemble-kernel-measurement.md`. |
