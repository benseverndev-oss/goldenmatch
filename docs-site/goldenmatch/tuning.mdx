...
| `GOLDENMATCH_FS_STRIP_HONORIFICS` | `1` (on) | Append the `strip_honorifics` transform to name comparison fields, so a title/rank token leaked into a name field ("Sir", "Baronet", "Bt.") stops carrying match weight (a honorific-only value becomes missing, not an empty-string agreement). Reaches the over-merge that `GOLDENMATCH_FS_TF_ADJUSTMENT` can't on historical_50k (F1 0.7520 -> 0.7652, precision +0.0245, recall +0.0005; beats Splink). The token set is surname-safe (ambiguous real surnames like Shah/King/Prince are excluded); regnal numerals are kept. Set `0` to restore the legacy behavior. |
