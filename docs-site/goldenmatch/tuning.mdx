...
| Variable | Default | Effect | When to set |
|----------|---------|--------|-------------|
| `GOLDENMATCH_SUGGEST_ON_DEDUPE` | on | Master switch for the default-pipeline healer surface. Set `0` to disable the trigger + attach on every `dedupe_df` / CLI / server run (the explicit `suggest=`/`heal=` opt-ins and `review_config` still work). | Set `0` to opt out of the advisory surface entirely. |
