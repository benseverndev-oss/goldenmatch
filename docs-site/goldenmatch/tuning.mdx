...
| `GOLDENMATCH_FS_ATOMIC_NAME_BLOCKING` | `off` | Atomic-name-soundex blocking completion. When names are blocked via a composite soundex pass (`full_name`/`first_and_surname`) but no atomic single-name soundex pass exists, one corrupted first name breaks the whole composite key — so same-surname duplicates never co-block. Adds an additive `surname`/`first_name` soundex pass (soundex, not strip: exact same-name blocks over-merge, but the fuzzy net's extra pairs are corrupted-spelling *true* matches). Measured leak-free B³: historical_50k 0.8620→0.8688 (+0.0067, recall +1.27pp, precision −0.0027); febrl3 no-op. `off` (default, pending the full ER panel); `auto` = person-shaped only (a `name` + a `date` col_type); `1`/`on` = everywhere. |
