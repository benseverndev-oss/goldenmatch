...
| `GOLDENMATCH_BLOCKING_COST_AWARE` | `0` | Demote a small-fixed-domain year/date column from the sole primary blocking key when a bounded name/compound fallback exists (it stays a recall pass), so identifier-poor data doesn't explode candidate pairs at scale. |
