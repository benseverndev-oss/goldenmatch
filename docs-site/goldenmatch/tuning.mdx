...
| `GOLDENMATCH_FS_PAIR_DEDUP` | `0` (off) | Opt-in **cross-pass candidate-pair dedup** for FS scoring. A `multi_pass` config emits the same within-block pair from every pass it co-blocks in, and the bucket scorer re-scores each copy (~46% redundant on historical_50k). Since an FS pair's score depends only on the two rows' comparison vector (not which pass co-blocked it), deduping + scoring each distinct pair once is **byte-identical** to the bucket route and faster (measured −13% to −36% wall, RSS flat-to-lower). Core FS path only — TF-adjustment / negative-evidence / (record_)embedding configs run the bucket route unchanged. `1` enables. |
| `GOLDENMATCH_FS_PAIR_DEDUP_MAX_PAIRS` | `200000000` | Memory guard for `GOLDENMATCH_FS_PAIR_DEDUP`: when the distinct candidate-pair set would exceed this many pairs, the pair-dedup scorer declines and the run falls through to the memory-bounded bucket route, so a pathological dense candidate set can't OOM. |
