...
| `GOLDENMATCH_ENSEMBLE_KERNEL` | unset (off) | **Opt-in.** When set to `1` (`on`/`true`) the `ensemble` scorer rides the bucket fast path (per-pair + the float64 `_score_block_vec` vec lane); by default it declines, routing the whole matchkey to the float32 `find_fuzzy_matches` matrix (byte-identical to historical behaviour). On Febrl3 the fast path measured **1.47× faster** end-to-end with byte-identical recall — but on **large fuzzy-only blocks** (e.g. an NCVR-shape weighted matchkey) keeping the matchkey on `score_buckets` makes each per-bucket worker spawn a *nested* `find_fuzzy_matches` ThreadPoolExecutor, which oversubscribes few-core machines and can be pathologically slow. So it stays opt-in until the nested-pool fix lands. | Leave unset. Set `1` only on workloads with small-to-mid blocks where you've measured a win. See `docs/superpowers/specs/2026-07-21-ensemble-kernel-measurement.md`. |
