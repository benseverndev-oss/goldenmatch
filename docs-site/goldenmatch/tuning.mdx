...
| `GOLDENMATCH_FIELD_GROUP_SURVIVORSHIP` | `0` (off) | Auto-detect correlated field groups for survivorship. Equivalent to `field_group_detection: true` in `golden_rules`. Off = byte-identical to prior behavior. |
