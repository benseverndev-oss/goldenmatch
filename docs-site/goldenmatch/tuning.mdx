...
| `GOLDENMATCH_FRAME` | `arrow` | `arrow`/`polars` | Frame backend. `arrow` is the DEFAULT since v3.0.0 (measured ~36% faster end-to-end on the 100K zero-config A/B): file ingest via pyarrow, the whole engine runs on the Frame seam, and result frames are `pyarrow.Table`. Since v3.1.0 polars is an OPTIONAL dependency: `polars` here requires `pip install 'goldenmatch[polars]'` (a clear error names the extra otherwise). Output-equivalence enforced by a differential harness + frozen fixtures. |
