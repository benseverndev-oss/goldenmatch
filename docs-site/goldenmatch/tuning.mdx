...
| `GOLDENMATCH_FS_U_PAIRS` | unset (`min(n_sample_pairs, 5000)`) | Random-pair budget for the FS `u` estimate. `u` is a level's frequency among random, overwhelmingly non-matching pairs; a level those pairs almost never reach (exact title/author agreement on bibliographic data) counts 0 in 5,000 pairs, so its `u` falls to the smoothing floor and its agreement weight explodes past the exact level's. Splink samples 1,000,000 by default and the Spark EM path uses `u_max_pairs=1_000_000`. Measured locally on dblp_acm: F1 0.3758 at the default → 0.8058 at 50,000 → 0.8242 at 200,000. The sample is scored pair by pair, so cost is a fixed per-run term. Measurement lever, default off (byte-identical). |
