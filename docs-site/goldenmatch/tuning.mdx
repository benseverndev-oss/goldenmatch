...
These tune the [healer](/goldenmatch/config-suggestions) — the `review_config` config-suggestion loop. They only matter when you call `review_config` (it is opt-in and needs `goldenmatch[native]`); the default `dedupe_df` pipeline does not read them. You almost never need to change these — the defaults are the values the suggester gym selected.
