...
| `GOLDENMATCH_FS_OOC_DUCKDB_MEMORY` | ~55% of system RAM | DuckDB `memory_limit` for the out-of-core FS connections (+ `temp_directory` next to the spilled db). Bounds DuckDB's buffer so the `prep` table SPILLS to disk and the sorted block scan streams, instead of DuckDB's default ~80%-of-RAM buffer holding the whole spilled table (an equivalent copy of the frame) resident — which would re-OOM at 50M. A DuckDB size string like `8GB`, or `0`/`off`/`default` to leave DuckDB's default. |
| `GOLDENMATCH_FS_OOC_GOLDEN_BATCH` | `500000` | Multi-member clusters the out-of-core golden build materializes per batch before streaming them to `golden.parquet` via a `ParquetWriter`. The golden subset can be MOST of N at full width, so building it whole is an O(N) resident spike; batching bounds the golden peak to one batch's member rows regardless of N (the same shape as the in-memory 25M golden fix). `0`/invalid → the 500K default. |
| `GOLDENMATCH_FS_OOC_SPILL_PAIRS` | `1` (on) | Within the out-of-core arrow-cluster path, SPILL the scored pair stream to the DuckDB file (a `pair_stream` table, appended per wave) instead of accumulating every wave's Arrow pair table in the driver. The max-score canonical dedup + best-score link filter then run in DuckDB, so only the bounded deduped edge set is materialized for the Union-Find — the Arrow-side twin of the prep spill, bounding the last O(pairs) driver structure (~2.4 GB at 100M pairs). `0` restores the in-RAM accumulation (the byte-parity oracle). |
| `GOLDENMATCH_FS_OOC_STREAM_CLUSTER` | `auto` | Out-of-core clustering route. `auto` uses the memory-bounded Rust streaming Union-Find (`StreamingClusterBuilder`) once the deduped edge set reaches `GOLDENMATCH_FS_OOC_STREAM_CLUSTER_MIN_PAIRS`, else the one-shot arrow kernel. `1` forces streaming; `0` forces the one-shot (the partition-parity oracle). Streaming feeds the DuckDB-spilled edges into a kernel that keeps only the Union-Find parent map (O(members)), never the O(pairs) deduped edge array (nor the one-shot kernel's `edge_pos`/`per_cluster_edges` metadata structures). Requires the native `StreamingClusterBuilder` symbol; an older wheel falls back to the one-shot path. Same connected-component partitions either way. |
| `GOLDENMATCH_FS_OOC_STREAM_CLUSTER_MIN_PAIRS` | `20000000` | `auto`-route threshold for the streaming Union-Find: use it once the deduped edge set reaches this many pairs (below it the one-shot fetch is cheap). |
| `GOLDENMATCH_FS_OOC_STREAM_CLUSTER_BATCH` | `2000000` | Edge-batch size for the streaming Union-Find scan — bounds the per-wave resident edge rows fed into the kernel. `0`/invalid → the 2M default. |
