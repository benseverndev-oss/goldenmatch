...
The `autoconfig` component is the shared, pyo3-free decision core (`goldenmatch-autoconfig-core`) behind auto-config — the planner, column classifier, the sample→full pair-count extrapolation (the corrected `ratio²` + Chao1 estimate), and the adaptive decision thresholds (the sparse-match floor). It runs native under `auto` and is byte-identical to the pure-Python oracle (golden-vector parity across Python / native / wasm-TS). The same crate compiles to WebAssembly so the TypeScript surface consumes the identical decision logic — one source of truth, zero cross-surface drift.
