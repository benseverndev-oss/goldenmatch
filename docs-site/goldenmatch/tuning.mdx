...
| `GOLDENMATCH_CLUSTER_SPLIT_TIES` | unset (one edge) | `level` makes the oversized-cluster auto-split cut every weakest spanning-tree edge that ties, instead of the first in endpoint order. Single-linkage at that height, so the split cannot depend on which tied edge sorts first. A component whose edges all tie still loses one edge, so a block of exact duplicates is not shattered. Measurement lever, default off. |
