...
| `GOLDENMATCH_SPARK_VECTOR_SCORER` | `0` | Score every comparison field of a pair in ONE UDF call instead of one call per field. The distributed FS counts stage is ~87% scoring against a ~0.1s kernel, so the per-call overhead dominates; this also lets fields sharing a scorer cross into native together (4 jaro-winkler + 1 levenshtein becomes 2 transitions, not 5). **Opt-in and unmeasured** -- it changes how every similarity on the Spark FS path is computed, and stays off until a measurement says it is faster. Falls back to the per-field path for `exact` fields (they never cross), unknown scorers, or more than 10 fields. |
