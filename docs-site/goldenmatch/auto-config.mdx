...
| `GOLDENMATCH_AUTOCONFIG_DETERMINISTIC=1` | Drop the search's wall-clock budgets so iteration caps are the only stopping rule. Without it the committed config depends on host speed and load. Runtime becomes unbounded: set it for anything that pins or gates auto-config output, not for interactive runs. |
