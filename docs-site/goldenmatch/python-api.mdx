...
class DedupeResult:
    ...
    native: NativeDispatchSummary | None  # v2.1.0: whether the scoring hot path used the Rust kernel or fell back

    deduplicated: pa.Table | None     # Property: one row per entity (golden + unique records)

    def to_csv(path, which="golden")  # Write results to CSV; which: "golden" | "deduplicated" | "dupes" | "unique" | "all"
    match_rate: float                 # Property: percentage of dupes
    total_records: int                # Property
    total_clusters: int               # Property: duplicate groups (clusters of 2+ records)
    total_entities: int               # Property: distinct entities = duplicate groups + unmatched records
```
...
```python
gm.run_graph_er(entities, relationships, propagation_mode="additive") -> dict
```
