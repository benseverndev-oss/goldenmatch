...
<Note>
  Blocking **auto-suggest chooses `token` for free-text columns by default.**
  Only columns that measure as free text reach it -- names and addresses sit
  below the token-count bar and keep their prefix/soundex keys -- so the plan
  changes exactly where a prefix key is near-useless. On Amazon-Google that is
  0.982 blocking recall against the previously committed key's 0.041.

  This shipped default-OFF at first, on a reading that committing it took that
  benchmark's F1 to 0.0000. It did not: nothing ever set `strategy="token"`, so
  the strategy never ran and the 0.0000 came from whatever config was committed
  instead. Set `GOLDENMATCH_TOKEN_BLOCKING=0` to force auto-suggest back off.
</Note>
