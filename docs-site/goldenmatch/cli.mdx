...
| Flag | Effect |
|------|--------|
| `--tui` | Open the interactive review TUI instead of running directly. `dedupe` is **non-interactive by default**: it auto-configs, writes the deduplicated list and golden records, and prints a summary. (`--no-tui` is accepted as a no-op for back-compat.) |
| `--original-values` / `--standardized-values` | Write unique, duplicate and deduplicated records with the values as entered, or as standardized. Unset: as entered for input up to 1,000,000 rows (keeping them costs memory for every column standardization rewrites). Golden records are always standardized. |
| `--output-deduplicated` | Write `<run>_deduplicated.csv`: one row per real-world entity (golden records plus the records that had no duplicate), each with its `__cluster_id__`. |
| `--output-all` / `--output-dir DIR` | Write every artifact (deduplicated/golden/clusters/dupes/unique/report) / choose the output directory. On the zero-config path, the deduplicated list and golden records are written by default. |
