...
| `goldenmatch import-splink SETTINGS.json -o CONFIG.yaml [--model-out MODEL.json]` | Convert a Splink settings or trained-model JSON to a GoldenMatch config; `--model-out` persists trained m/u probabilities for reuse. |
