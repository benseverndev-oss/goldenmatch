---
title: "GoldenMatch API quick reference"
description: "Practical examples for the most-used surface. Authoritative type signatures live in goldenmatch/_api.py and goldenmatch/config/schemas.py."
...
result.deduplicated    # pa.Table | None - one row per entity: golden records + unique records, each with __cluster_id__
# unique / dupes / deduplicated hold the values as entered (golden stays standardized);
# keep_original_values=True|False forces it, unset decides by size (always for dedupe_df)
