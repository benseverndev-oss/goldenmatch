---
title: "REST API"
description: "Local HTTP server for real-time matching, cluster browsing, and data steward review."
keywords: ["rest api", "http server", "matching", "clusters", "review queue"]
---
...
The server is fail-closed: binding to a non-loopback host refuses to start
unless `GOLDENMATCH_API_TOKEN` is set. When the token is set, every endpoint
except `GET /health` requires it as a bearer token.

```bash
export GOLDENMATCH_API_TOKEN="your-secret-token"
goldenmatch serve --file customers.csv --host 0.0.0.0 --port 8080

curl -H "Authorization: Bearer your-secret-token" http://host:8080/stats
```

Cross-origin requests are denied by default (no wildcard). To allow specific
browser origins, set a comma-separated allowlist:

```bash
export GOLDENMATCH_API_CORS_ORIGINS="https://app.example.com,https://admin.example.com"
```
...
```json
{"status": "ok", "service": "goldenmatch", "ready": true}
```

Returns `503` with `"status": "initializing"` until the initial matching run
has populated results — readiness, not just liveness.
