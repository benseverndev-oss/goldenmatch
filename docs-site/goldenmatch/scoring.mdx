...
The probabilistic auto-config path (`auto_configure_probabilistic_df` / `build_probabilistic_matchkeys`) now builds Fellegi-Sunter configs that beat Splink on every dataset in the shared `bench_er_headtohead` panel. This is **default-on** as of FS auto-config v2; set `GOLDENMATCH_FS_AUTOCONFIG_V2=0` to restore the legacy auto-config byte-identically. It touches only the probabilistic path — the weighted/DQbench path and zero-config `dedupe_df` are unchanged.

| Dataset | GoldenMatch (probabilistic v2) | Splink |
|---------|--------------------------------|--------|
| historical_50k (Splink's flagship) | 0.779 | 0.757 |
| febrl3 | 0.991 | 0.965 |
| synthetic_person | 0.998 | 0.996 |
| dblp_acm (bibliographic) | 0.879 | (skips) |
