...
- **Splink migration upgrade pass.** `goldenmatch import-splink settings.json --upgrade data.parquet -o goldenmatch.yaml --model-out model.json` (or `gm.upgrade_splink_conversion(conversion, data)`) runs a data-aware pass over the converted config: it computes term-frequency tables from the data (Splink model exports don't include them), re-derives Levenshtein distance thresholds from measured string lengths, and calibrates link/review thresholds from the blocked-pair score distribution. The faithful baseline conversion is always written alongside as `*.baseline.*`, and a baseline-vs-upgraded delta table is printed (add `--splink-clusters` / `--labels` for agreement / truth F1). Measured on wild Splink configs, the upgraded conversion beat native Splink on all three benchmark pairs (pairwise F1 vs truth: 0.633 vs 0.601, 0.766 vs 0.699, 0.740 vs 0.686).
