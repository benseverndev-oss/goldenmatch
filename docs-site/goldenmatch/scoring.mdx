...
| `array_intersect` | Token-set overlap of delimited strings (`:jaccard` / `:overlap`) | 0.0--1.0 | Delimited lists (tags, categories, phones) |
| `cosine` | Cosine of two **precomputed** float-vector columns (parsed from a delimited/bracketed string); direct, not model-embedded like `embedding` | 0.0--1.0 | Precomputed embedding columns |
...
- **`array_intersect`** splits each side on delimiters (`|`, `;`, `,`) into a token set and scores their overlap. Bare `array_intersect` = Jaccard (`|A∩B| / |A∪B|`); `array_intersect:overlap` = the overlap coefficient (`|A∩B| / min(|A|,|B|)`). An empty side falls back to exact-string equality. Compares delimited lists (tags, categories, multi-valued phones) as sets rather than as raw strings, so member reordering no longer reads as disagreement.
...
- **Link cutoff placement.** Under linear scoring the link cutoff is placed by an evidence-bits rule, not a fixed 0.50 midpoint. The default rule is `prior_mid`, which links at `W >= max(log2((1-λ)/λ), midpoint, 0)`. The cut router (`GOLDENMATCH_FS_CUT_ROUTER`, on by default) then picks a rule per matchkey from the trained model's cut diagnostics. Pin a rule per matchkey with `link_cut_rule` (`midpoint`, `prior`, `prior_mid`, `posterior_099`, `evidence_3`/`5`/`9`/`12`, `otsu`), or process-wide with `GOLDENMATCH_FS_LINEAR_CUT`; `GOLDENMATCH_FS_LINEAR_CUT=off` restores the midpoint. An explicit `link_threshold`, a calibrated cutoff and posterior scoring all take precedence. Each matchkey's `stats["fs_link_thresholds"]` entry reports which rule decided (`cut_rule`) and why (`cut_reason`). Since 3.18.0.
...
- **Admit `description` / multi-name fields as `token_sort`**, which lifts the bibliographic case (DBLP-ACM) from 0.003 → 0.381. A large relative gain, but still recall-bound.
...
<Note>
  **For bibliographic data (DBLP-ACM), use the weighted path, not probabilistic.** Splink skips dblp_acm, and the probabilistic auto-config is weak there (pairwise F1 0.381, recall-bound). The zero-config **weighted** controller scores 0.964 on DBLP-ACM and is the right tool for that shape. The probabilistic path targets PII / person-record linkage (historical_50k, febrl3, synthetic_person).
</Note>
