...
- **SQLite ceiling.** SQLite is the right default through the low millions of rows on a single box. Past that, prefer Postgres: it gets a bulk `COPY` path for brand-new identities that SQLite has no equivalent for, and it is the only backend the distributed (Ray) resolver supports. The tradeoff is network latency per run, not a throughput cliff.
