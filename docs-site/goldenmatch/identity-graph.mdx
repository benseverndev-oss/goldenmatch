...
identity:
  ...
  source_pk_column: null   # column to derive {source}:{source_pk} record_id
                           # when null, derives {source}:h1:{12 hex} from the
                           # row fingerprint (un-fingerprintable rows fall back
                           # to {source}:hash:{12 hex} from the payload SHA-256)
...
- **`emit_singletons` is the single biggest cost lever, and it defaults to `true`.** With it on, every single-record cluster becomes its own identity, so resolve work scales with **row count** rather than with the number of real multi-record entities — on a 14M-row dedupe that is 14M identities, not ~500k. Turn it off unless you specifically need a durable id for records that matched nothing:
  ...
  With `emit_singletons: false` the resolver also skips preparing rows no multi-record cluster references, so both wall time and resident memory drop with it.

  **Concrete ceiling for `emit_singletons: true`.** Because it can't bound the prep to referenced rows (every row becomes an identity), the `true` path scales with the input frame and is only practical to roughly **100k rows** on SQLite — measured, a 250k-row dedupe was still running at 25+ minutes, whereas the same ladder with `emit_singletons: false` finished 250k in ~42s and 1M in ~6 min. Past ~100k rows, resolve logs a one-time warning pointing you at `emit_singletons: false`. If you genuinely need a durable id for every unmatched record above that scale, use `backend: postgres` (bulk `COPY`) rather than SQLite.
- **SQLite ceiling.** SQLite is the right default through the low millions of rows on a single box. Past that, prefer Postgres: brand-new identities take a bulk `COPY` fast path there, and it is the only backend the distributed (Ray) resolver supports. The tradeoff is network latency per run, not a throughput cliff.

  **The distributed (per-partition) resolver is not reachable from a single-node SQLite run.** `goldenmatch.distributed.identity.resolve_identities_distributed` requires a Ray cluster **and** `backend: postgres` (it materializes cluster aggregates on the driver and resolves against a pooled Postgres connection; true per-partition `map_batches` is still a roadmap item). So a single-node in-memory run has exactly two scale levers: keep `emit_singletons: false` so resolve tracks the identity graph rather than the frame, and — past the low millions — switch to `backend: postgres`. There is no single-node distributed/partitioned SQLite path.

  The SQLite gap is in *our* write path, not in SQLite: the stdlib `sqlite3` driver has no `COPY` equivalent, so resolve writes go row-at-a-time (batched into transactions) rather than as columnar bulk ingest. Arrow-native bulk ingest into SQLite *is* available — the [Apache Arrow ADBC SQLite driver](https://arrow.apache.org/adbc/current/driver/sqlite.html) supports `adbc_ingest` from Python and can be driven from Rust via `adbc_core` + `adbc_driver_manager` — and adopting it would let the SQLite backend share the same staging-table-then-upsert shape the Postgres bulk path uses. That is a planned follow-up, not something GoldenMatch does today.
...
- [Pipeline architecture](/concepts/architecture) — where identity sits in the dedupe flow
- [Learning Memory](/goldenmatch/learning-memory) — the other persistent-state layer; complementary, not competing
- [Configuration](/goldenmatch/configuration) — full schema reference
