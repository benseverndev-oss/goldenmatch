...
```bash
pip install goldenmatch[polars]         # optional COMPATIBILITY extra: the classic GOLDENMATCH_FRAME=polars lane + kernel-absent golden replay + cell-quality weighting. NOT needed for speed -- the polars-free install measures faster (the Rust kernels carry the hot paths).
