...
```bash
pip install goldenmatch[polars]         # optional: polars wall-optimizations + the classic GOLDENMATCH_FRAME=polars lane (v3.1.0+: polars is no longer a required dependency)
