...
209 `GOLDENMATCH_*` runtime knob(s) read by the package, scanned from source so this is complete. Details in [Tuning & opt-ins](/goldenmatch/tuning). Grouped by area:
...
- **IDENTITY** (9): `GOLDENMATCH_IDENTITY_BATCH_FINGERPRINT`, `GOLDENMATCH_IDENTITY_BULK`, `GOLDENMATCH_IDENTITY_BULK_FLUSH_ROWS`, `GOLDENMATCH_IDENTITY_DSN`, `GOLDENMATCH_IDENTITY_INITIAL_LOAD`, `GOLDENMATCH_IDENTITY_INITIAL_LOAD_UNLOGGED`, `GOLDENMATCH_IDENTITY_SQLITE_BATCH`, `GOLDENMATCH_IDENTITY_SQLITE_BATCH_SIZE`, `GOLDENMATCH_IDENTITY_WRITE_PIPELINE`
