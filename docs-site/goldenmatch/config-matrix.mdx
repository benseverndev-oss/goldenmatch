...
| `keep_original_values` | bool \| None | `None` | Return unique/duplicate/deduplicated records with the values as entered rather than standardized; unset decides by input size. |
...
| `guard` | str \| None | `None` | Pair predicate (over a_&lt;col>/b_&lt;col>) gating whether this whole matchkey fires on a pair. |
...
| `link_threshold_observed` | float \| None | `None` | The link cutoff a run actually resolved, recorded for reporting and for threshold perturbation. Never read when resolving thresholds, so it does not pin the cutoff or skip the full-data refit the way link_threshold does. |
| `link_cut_rule` | Literal \| None | `None` | `midpoint`, `prior`, `prior_mid`, `posterior_099`, `evidence_3`, `evidence_5`, `evidence_9`, `evidence_12`, `otsu` -- Rule that places the probabilistic link cutoff by evidence bits, for example prior_mid or evidence_9. Unset lets the link-cut router pick the rule (GOLDENMATCH_FS_CUT_ROUTER), else the default rule; link_threshold, when set, still wins. |
...
| `strategy` | Literal | `'static'` | `static`, `adaptive`, `sorted_neighborhood`, `multi_pass`, `ann`, `canopy`, `ann_pairs`, `learned`, `lsh`, `simhash`, `perceptual`, `token` -- Candidate-generation method that selects how pairs are proposed for scoring. |
...
| `backend` | str | `'sqlite'` | Storage backend for the identity graph ('sqlite', 'postgres', 'mongo' or 'snowflake'). |
| `path` | str | `'.goldenmatch/identity.db'` | SQLite file path for the identity graph when the backend is sqlite. |
| `connection` | str \| dict \| None | `None` | Connection for non-sqlite backends: a DSN string for postgres, a URI for mongo, or an account name / connector-kwargs dict for snowflake. A Snowpark Session is passed programmatically, not through config. |
| `database` | str | `'goldenmatch'` | Database holding the identity tables for the snowflake backend, and the database name for the mongo backend. |
| `schema_` _(alias `schema`)_ | str | `'PUBLIC'` | Snowflake schema holding the identity tables. |
...
| `guard` | str \| None | `None` | Pair predicate (over a_&lt;col>/b_&lt;col>) gating whether this field contributes; weighted matchkeys only. |
...
| `additive` | bool | `False` | If true, this pass co-locates pairs but its fields remain EM-trained comparison fields (Fellegi-Sunter path) instead of being demoted to fixed neutral weights. |
...
### `TokenBlockingConfig`
_DF-pruned token blocking on a free-text column (#2488)._

| Field | Type | Default | Choices / notes |
|---|---|---|---|
| `column` | str | **required** | Free-text column tokenized to build the inverted index. |
| `min_token_length` | int | `3` | Tokens shorter than this are dropped; short tokens are mostly noise ('1', 'w/') and form large low-evidence blocks. |
| `max_df_ratio` | float | `0.02` | Drop tokens appearing in more than this fraction of records. Used to derive the cap when max_df is unset. |
| `max_df` | int \| None | `None` | Absolute document-frequency cap. When unset it is derived from max_df_ratio and clamped to [10, 1000] so the cap stays bounded at any frame size. |
...
| Field | Type | Default | Choices / notes |
|---|---|---|---|
| `field` | str | **required** | Payload field whose shared value relates two entities (e.g. 'phone_number', 'zip5', 'employer', 'email', 'specialty', 'degree'). |
| `kind` | str | **required** | Relationship label for the emitted edge (e.g. 'shares_phone', 'same_address'). |
| `transform` | str \| None | `None` | Optional derived-value transform applied to `field` before grouping, so edges key on a DERIVED value rather than the literal field: 'email_domain' (the domain after '@'), 'normalize_company' (lowercased, common corp suffixes stripped), 'lower_trim' (lowercased + trimmed, good for specialty/degree), 'zip3' (first 3 chars of a zip). None (default) uses the raw field value. |
...
| `dedupe` | `--preview-random` | boolean | `False` | Deprecated no-op: accepted for back-compat but not applied (preview uses the first N). |
| `dedupe` | `--output-golden` | boolean | `False` | Output golden records |
| `dedupe` | `--original-values` | boolean | `None` | Write unique/duplicate/deduplicated records with the values as entered, or as standardized. Unset: as entered up to 1,000,000 input rows. |
| `dedupe` | `--output-deduplicated` | boolean | `False` | Output the deduplicated list: one row per entity (golden records + unique records) |
...
| `dedupe` | `--chunked` | boolean | `False` | Deprecated no-op: use --backend chunked for large-dataset chunked processing. |
...
| `dedupe` | `--diff-html` | boolean | `False` | Deprecated no-op: accepted for back-compat but not generated; use --diff for a CSV diff. |
...
| `dedupe` | `--anomaly-sensitivity` | text | `'medium'` | Deprecated no-op: accepted for back-compat but not applied (anomaly sensitivity is fixed). |
...
| `discover-model` | `--apply-names` | boolean | `False` | Write the namer's VERIFIED names into the emitted YAML (entity/measure label:, dimension/value meta.goldenmatch.glossary). Implies --name; post-certification + cosmetic. |
...
| `identity 360` | `entity_id` | text | **required** | The entity_id to build the Customer 360 view for. |
| `identity 360` | `--path` | text | `'.goldenmatch/identity.db'` | Path to the identity graph database. |
...
| `match` | `--preview-random` | boolean | `False` | Deprecated no-op: accepted for back-compat but not applied (preview uses the first N). |
...
| `score_strings` | Score similarity between two strings using the requested scorer. |
...
| `array_intersect` | Token-set overlap of delimited strings (Jaccard by default; 'array_intersect:overlap' for the overlap coefficient). FS path. | Delimited lists (tags, categories, phones) | 0.0-1.0 |
| `audio_fp` | Audio-fingerprint similarity. | Audio clips | 0.0-1.0 |
| `cosine` | Cosine similarity of two PRECOMPUTED float-vector columns (parsed from a delimited/bracketed string); direct, not model-embedded like 'embedding'. | Precomputed embedding columns | 0.0-1.0 |
...
| `token` | Inverted index over DF-pruned tokens; a record joins one block per token, so a pair is a candidate when it shares any token. | Free text where no prefix agrees (product titles) |
...
219 `GOLDENMATCH_*` runtime knob(s) read by the package, scanned from source so this is complete. Details in [Tuning & opt-ins](/goldenmatch/tuning). Grouped by area:
...
- **AUTOCONFIG** (11): `GOLDENMATCH_AUTOCONFIG_ARROW_NATIVE`, `GOLDENMATCH_AUTOCONFIG_DETERMINISTIC`, `GOLDENMATCH_AUTOCONFIG_FORCE_EXCLUDE`, `GOLDENMATCH_AUTOCONFIG_FORCE_INCLUDE`, `GOLDENMATCH_AUTOCONFIG_INDICATOR_BUDGET`, `GOLDENMATCH_AUTOCONFIG_LLM`, `GOLDENMATCH_AUTOCONFIG_MEMORY`, `GOLDENMATCH_AUTOCONFIG_ROUTE_PROBABILISTIC`, `GOLDENMATCH_AUTOCONFIG_SAMPLE_STRATEGY`, `GOLDENMATCH_AUTOCONFIG_ZERO_LABEL_COMMIT`, `GOLDENMATCH_AUTOCONFIG_ZERO_LABEL_STABILITY`
- **BASE** (1): `GOLDENMATCH_BASE_STORE`
- **BENCH** (1): `GOLDENMATCH_BENCH_DUMP_PAIRS`
- **BLOCKING** (10): `GOLDENMATCH_BLOCKING_CARDINALITY_SCALER`, `GOLDENMATCH_BLOCKING_COST_AWARE`, `GOLDENMATCH_BLOCKING_DEGENERATE_MAX_AVG_BLOCK_SIZE`, `GOLDENMATCH_BLOCKING_DEGENERATE_THRESHOLD`, `GOLDENMATCH_BLOCKING_DIVERSIFY_PAIR_BUDGET`, `GOLDENMATCH_BLOCKING_MAX_RATIO`, `GOLDENMATCH_BLOCKING_MIN_RATIO`, `GOLDENMATCH_BLOCKING_PAIRS_PER_ROW`, `GOLDENMATCH_BLOCKING_PASS_MIN_WEAKPOS`, `GOLDENMATCH_BLOCKING_PRUNE_PASSES`
...
- **CLUSTER** (3): `GOLDENMATCH_CLUSTER_DEBUG`, `GOLDENMATCH_CLUSTER_SPLIT_EDGE_BUDGET`, `GOLDENMATCH_CLUSTER_SPLIT_TIES`
...
- **DISTRIBUTED** (14): `GOLDENMATCH_DISTRIBUTED_BLOCK_SHUFFLE`, `GOLDENMATCH_DISTRIBUTED_CLUSTERING_THRESHOLD`, `GOLDENMATCH_DISTRIBUTED_FS_TRAIN_ROWS`, `GOLDENMATCH_DISTRIBUTED_GOLDEN_THRESHOLD`, `GOLDENMATCH_DISTRIBUTED_ON_ERROR`, `GOLDENMATCH_DISTRIBUTED_OP_RESERVATION`, `GOLDENMATCH_DISTRIBUTED_PIPELINE`, `GOLDENMATCH_DISTRIBUTED_SCORE_CONCURRENCY`, `GOLDENMATCH_DISTRIBUTED_SCORE_NUM_CPUS`, `GOLDENMATCH_DISTRIBUTED_SCORE_PROJECT`, `GOLDENMATCH_DISTRIBUTED_SHUFFLE_PARTS`, `GOLDENMATCH_DISTRIBUTED_WCC`, `GOLDENMATCH_DISTRIBUTED_WCC_SCRATCH`, `GOLDENMATCH_DISTRIBUTED_WCC_SEED`
...
- **FS** (52): `GOLDENMATCH_FS_ARROW_STREAM`, `GOLDENMATCH_FS_ATOMIC_NAME_BLOCKING`, `GOLDENMATCH_FS_AUTOCONFIG_V2`, `GOLDENMATCH_FS_BATCH_ROWS`, `GOLDENMATCH_FS_BLOCK_SOURCE`, `GOLDENMATCH_FS_BUCKET_BATCH`, `GOLDENMATCH_FS_BUCKET_NATIVE`, `GOLDENMATCH_FS_BUCKET_SCORE_WORKERS`, `GOLDENMATCH_FS_CALIBRATED`, `GOLDENMATCH_FS_CALIBRATE_THRESHOLD`, `GOLDENMATCH_FS_COLUMNAR_CLUSTER`, `GOLDENMATCH_FS_CUT_ROUTER`, `GOLDENMATCH_FS_DEFAULT_BUCKET`, `GOLDENMATCH_FS_DOMAIN_COMPARATORS`, `GOLDENMATCH_FS_EM_AGG_BLOCKS`, `GOLDENMATCH_FS_EM_BLOCK_SLIM`, `GOLDENMATCH_FS_EM_COUNTED`, `GOLDENMATCH_FS_EM_SAMPLE_ROWS`, `GOLDENMATCH_FS_EVIDENCE_CUT`, `GOLDENMATCH_FS_FACTORIZE`, `GOLDENMATCH_FS_FACTORIZE_MIN_REPEAT`, `GOLDENMATCH_FS_FD_CLASSES`, `GOLDENMATCH_FS_FD_PROBE`, `GOLDENMATCH_FS_FIELD_DEPENDENCE`, `GOLDENMATCH_FS_LINEAR_CUT`, `GOLDENMATCH_FS_MAX_PASS_PAIRS`, `GOLDENMATCH_FS_MISSING`, `GOLDENMATCH_FS_MONOTONIC`, `GOLDENMATCH_FS_NATIVE`, `GOLDENMATCH_FS_OOC_ARROW_CLUSTER`, `GOLDENMATCH_FS_OOC_DEBUG`, `GOLDENMATCH_FS_OOC_WAVE_ROWS`, `GOLDENMATCH_FS_ORTHOGONAL_BLOCKING`, `GOLDENMATCH_FS_OUT_OF_CORE`, `GOLDENMATCH_FS_OVERSIZED_CHUNK_PAIRS`, `GOLDENMATCH_FS_POST_BLOCKING_U`, `GOLDENMATCH_FS_REFIT_SWEEP`, `GOLDENMATCH_FS_REFIT_THRESHOLD`, `GOLDENMATCH_FS_REQUIRE_POSITIVE_EVIDENCE`, `GOLDENMATCH_FS_ROUTE_MIN_ROWS`, `GOLDENMATCH_FS_SCORED_PAIRS_MAX`, `GOLDENMATCH_FS_SEQUENTIAL_BATCHED_OUTPUT`, `GOLDENMATCH_FS_SEQUENTIAL_BOUNDED_SCORING`, `GOLDENMATCH_FS_SIGNATURE_PRUNE`, `GOLDENMATCH_FS_SPILL_FORCE_SHARD`, `GOLDENMATCH_FS_STRIP_HONORIFICS`, `GOLDENMATCH_FS_TF_ADJUSTMENT`, `GOLDENMATCH_FS_TF_CLAMP`, `GOLDENMATCH_FS_U_PAIRS`, `GOLDENMATCH_FS_VECTORIZED`, `GOLDENMATCH_FS_VEC_MAX_ELEMS`, `GOLDENMATCH_FS_WORKERS`
...
- **IDENTITY** (10): `GOLDENMATCH_IDENTITY_BATCH_FINGERPRINT`, `GOLDENMATCH_IDENTITY_BULK`, `GOLDENMATCH_IDENTITY_BULK_FLUSH_ROWS`, `GOLDENMATCH_IDENTITY_DSN`, `GOLDENMATCH_IDENTITY_INITIAL_LOAD`, `GOLDENMATCH_IDENTITY_INITIAL_LOAD_UNLOGGED`, `GOLDENMATCH_IDENTITY_PREP_CHUNK_ROWS`, `GOLDENMATCH_IDENTITY_SQLITE_BATCH`, `GOLDENMATCH_IDENTITY_SQLITE_BATCH_SIZE`, `GOLDENMATCH_IDENTITY_WRITE_PIPELINE`
- **INHOUSE** (1): `GOLDENMATCH_INHOUSE_MODEL`
- **KEY** (1): `GOLDENMATCH_KEY_INTEGRITY_NATIVE`
- **LLAMA** (1): `GOLDENMATCH_LLAMA_GGUF`
- **LLM** (2): `GOLDENMATCH_LLM_BASE_URL`, `GOLDENMATCH_LLM_MODEL`
- **LOCAL** (4): `GOLDENMATCH_LOCAL_LLM`, `GOLDENMATCH_LOCAL_LLM_CACHE`, `GOLDENMATCH_LOCAL_LLM_GPU_LAYERS`, `GOLDENMATCH_LOCAL_LLM_PATH`
...
- **SEMANTIC** (2): `GOLDENMATCH_SEMANTIC_BLOCKING_THRESHOLD`, `GOLDENMATCH_SEMANTIC_NAMER`
- **SKIP** (1): `GOLDENMATCH_SKIP_MATCH_LOG`
- **SPARK** (4): `GOLDENMATCH_SPARK_EXACT_PROFILING`, `GOLDENMATCH_SPARK_FUSED_BLOCK_JOIN`, `GOLDENMATCH_SPARK_JAR`, `GOLDENMATCH_SPARK_VECTOR_SCORER`
...
- **TOKEN** (1): `GOLDENMATCH_TOKEN_BLOCKING`
- **TRANSFORM** (1): `GOLDENMATCH_TRANSFORM_DEBUG`
- **TRANSITIVE** (2): `GOLDENMATCH_TRANSITIVE_POSTFLIGHT`, `GOLDENMATCH_TRANSITIVE_WEAK_MARGIN`
