...
206 `GOLDENMATCH_*` runtime knob(s) read by the package, scanned from source so this is complete. Details in [Tuning & opt-ins](/goldenmatch/tuning). Grouped by area:
...
- **SPARK** (3): `GOLDENMATCH_SPARK_FUSED_BLOCK_JOIN`, `GOLDENMATCH_SPARK_JAR`, `GOLDENMATCH_SPARK_VECTOR_SCORER`
