...
| `import-dbt` | `manifest` | str | **required** | dbt manifest.json (from `dbt compile` / `dbt parse`) |
| `import-dbt` | `--output` | str | `'goldenmatch.yaml'` | Output YAML config path |
| `import-dbt` | `--catalog` | str | `None` | dbt catalog.json (from `dbt docs generate`); supplies model column lists so recognized most-recent survivorship is emitted as golden_rules (otherwise it is only reported) |
...
| `import-dbt` | `--verify` | str | `None` | Prove the distillation reproduces your dbt pipeline: run the converted config on a sample of --source and report pairwise cluster agreement against this output table (parquet/csv; first two columns = id, cluster_id). Needs --source. |
| `import-dbt` | `--source` | str | `None` | Source rows the dbt ER model consumed (parquet/csv), for --verify |
| `import-dbt` | `--verify-sample` | int | `5000` | Rows of --source to run through GoldenMatch for --verify |
| `import-dbt` | `--id-column` | str | `None` | Column holding each source row's id (default: auto-detect unique_id/id/record_id) |
...
| `import-splink` | `--verify` | str | `None` | Prove the conversion preserved behaviour: run BOTH the original Splink settings (needs `pip install splink`) AND the converted config on a sample of this dataset (parquet/csv) and report pairwise cluster agreement. Best-effort -- skipped with a notice if splink is absent or the settings can't run under DuckDB. |
| `import-splink` | `--verify-sample` | int | `5000` | Rows to run through both engines for --verify (seeded subsample above it) |
...
| `llm serve-local` | `--host` | str | `'127.0.0.1'` | Bind host (loopback by default). |
| `llm serve-local` | `--port` | int | `8081` | Bind port. |
| `llm serve-local` | `--model-path` | str | `None` | Explicit GGUF path (else the pinned model is resolved / downloaded). |
...
| `migrate-splink` | `input_path` | str | **required** | Splink settings or trained-model JSON file |
| `migrate-splink` | `data` | str | **required** | Data to deduplicate (parquet/csv) |
| `migrate-splink` | `--output` | str | `'clusters.parquet'` | Where to write the deduplicated (golden) records (parquet/csv) |
| `migrate-splink` | `--config-out` | str | `None` | Also write the converted GoldenMatch YAML config here |
...
| `migrate-splink` | `--verify-sample` | int | `5000` | Rows to run through both engines for --verify |
...
| `migrate-splink` | `--id-column` | str | `None` | Column holding each row's id (default: auto-detect unique_id/id/record_id) |
...
187 `GOLDENMATCH_*` runtime knob(s) read by the package, scanned from source so this is complete. Details in [Tuning & opt-ins](/goldenmatch/tuning). Grouped by area:
