...
This auto-detects column types, assigns scorers, picks a blocking strategy, runs, and writes the deduplicated list (a timestamped `*_deduplicated.csv`, one row per real-world entity) and golden records (`*_golden.csv`, one merged record per duplicate group) in the current directory, no config, no prompts. Add `--tui` to review interactively instead, or `--output-all` for every artifact. The same in Python:
