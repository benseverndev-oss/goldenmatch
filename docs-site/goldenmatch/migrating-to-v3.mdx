...
blocked). Installing the `[polars]` extra lights up the polars-present wall
optimizations (golden fast-columnar, vectorized survivorship, the large-scale
pair-score join) and the classic `GOLDENMATCH_FRAME=polars` lane, byte-identical
to 3.0.x behavior.
