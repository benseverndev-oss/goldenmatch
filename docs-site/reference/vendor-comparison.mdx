...
| AI-native surface | 60+ MCP tools, agent skills, A2A, REST API. |

## OSS engines

| Engine | License | Where it beats GoldenMatch | Where GoldenMatch beats it |
|--------|---------|----------------------------|----------------------------|
| Splink (UK MoJ) | MIT | Distributed Fellegi-Sunter at 1B+ rows (Spark); mature interactive m/u + comparison-viewer charting UI. | Non-PII accuracy (DBLP-ACM 0.964 vs 0.728); zero-config; polyglot; AI-native; PPRL. **Probabilistic head-to-head closed and reversed:** under the shared `bench_er_headtohead` evaluator (pairwise F1), GoldenMatch's probabilistic (Fellegi-Sunter) auto-config beats Splink on every dataset Splink scores — historical_50k (Splink's flagship) 0.827 vs 0.757, febrl3 0.996 vs 0.965, synthetic_person 1.000 vs 0.996 (and at the cluster level on historical_50k, B-cubed F1 0.862 vs 0.788). These are **deterministic** as of #829 (numbers refreshed 2026-08-06). *(Pairwise F1 under one shared evaluator; the often-cited ~0.97 on historical_50k is a cluster/entity-level metric, not exhaustive within-cluster pairwise F1 — Splink itself is ~0.76 pairwise here under the same harness. Splink skips bibliographic dblp_acm; the probabilistic path is weak there (0.381) — use the weighted controller, which scores 0.964. Full bake-off + walkthrough: the [zero-config vs. Splink case study](/case-studies/zero-config-vs-splink).)* The `type: probabilistic` matchkey also has EM-trained m/u, supervised m from labels, model persistence, match-weight waterfall, posterior calibration, and label-driven threshold analysis, plus FS scale-out on the bucket/Ray path (measured 6M dedupe in 162.6 s / 11.3 GB single-node). |
