...
|---------|:-------------:|:----------------:|-----|:---------:|:----:|:---:|--------------|
| [GoldenMatch](#goldenmatch) | `3.9.0` | `1.23.0` | `goldenmatch` | 87 | ✓ | ✓ | wheel · Postgres · DuckDB |
...
- **Python is the fuller *stateful* server.** It holds datasets, runs, clusters, and
  the identity graph server-side. That head start has largely been closed — run
  *mutation* (`unmerge_record`, `shatter_cluster`), incremental resolution, the
  `identity_*` tools, and domain packs are all **shared** now. What remains Python-only
  is the set with no JS-ecosystem equivalent: VLM document ingest, the distributed
  routing planner, entry-point plugin discovery, and PPRL auto-config.
- **TypeScript is the *edge-first* surface.** Its edge-safe core `dedupe()` returns
  ...
  the node-only MCP server, not the edge-safe core.) The remaining TS-only tools are
  the three host helpers `read_file`, `write_csv`, and `server_info`. (The scoring /
  blocking / clustering primitives `score_strings`, `score_pair`, `find_exact_matches`,
  `find_fuzzy_matches`, and `build_clusters` were TS-only until 2026-07-24; Python now
  exposes all five, so they are shared.)

So GoldenMatch's **83 TS tools vs 87 Python** breaks down as 80 shared operations,
3 TS-only host helpers, and 7 Python-only tools (the VLM document-ingest pair, the
distributed routing trio, `list_plugins`, and `pprl_auto_config`) — overlapping
surfaces sized to their jobs, not a subset. On the shared core the **public function
names match**, and that parity
...
web UI (`goldenmatch serve-ui`) · **87 MCP tools**.
