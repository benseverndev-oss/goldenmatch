...
|---------|:-------------:|:----------------:|-----|:---------:|:----:|:---:|--------------|
| [GoldenMatch](#goldenmatch) | `3.19.0` | `1.29.0` | `goldenmatch` | 97 | ✓ | ✓ | wheel · Postgres · DuckDB |
| [GoldenCheck](#goldencheck) | `3.6.0` | `0.9.0` | `goldencheck` | 19 | ✓ | ✓ | wheel |
...
- **TypeScript is the *edge-first* surface.** Its edge-safe core `dedupe()` returns
  ...
  the node-only MCP server, not the edge-safe core.) **As of 2026-07-24 nothing is
  TS-only** — the scoring/blocking/clustering primitives (`score_strings`, `score_pair`,
  `find_exact_matches`, `find_fuzzy_matches`, `build_clusters`) and the host helpers
...
<Note>
  `read_file` and `write_csv` are host conveniences, not ER capabilities. Both route
  through the same `safe_path` guard as every other file-taking tool, which enforces
  containment **only when `GOLDENMATCH_ALLOWED_ROOT` is set**. Set it (alongside
  `GOLDENMATCH_MCP_TOKEN`) when exposing an MCP server to untrusted callers.
</Note>
