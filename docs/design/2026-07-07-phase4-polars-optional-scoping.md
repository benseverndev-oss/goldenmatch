...
| **Parquet I/O** | **CLOSED (Polars-free)** | `transform("<x>.parquet")` reads via pyarrow `to_pydict()` (typed dict — value-identical to `pl.read_parquet`), byte-identical to `transform_df(pl.read_parquet(...))` for configured AND zero-config. Needs `[parquet]` (pyarrow only, no polars). |
| **Excel I/O** | **CLOSED (Polars-free)** | `transform("<x>.xlsx")` reads via openpyxl (the `[excel]` dep — *lighter* than the `fastexcel` `pl.read_excel` needs) into a typed dict. openpyxl reads whole-number floats as int (a documented, arguably-more-natural edge vs fastexcel). |
| **DB I/O** | **CLOSED (Polars-free)** | `connectors.database.read_database_columns(conn, query)` reads any PEP-249 (DBAPI) connection into a typed dict — stdlib DBAPI, no connectorx/polars — feeding straight into `transform()`. |
| **CSV zero-config** | **CLOSED (owned)** | `transform("<x>.csv", config=None)` runs Polars-free by profiling columns **as text** (their on-disk form) — deliberately NOT coercing numeric-looking IDs/zips to ints (`"01234"` stays a zip, the data-cleaning-correct choice the TS port already made). A dates-style *own-the-behavior* decision: the cleaned **text** values match `transform_df(pl.read_csv(...), config=None)`; numeric columns keep their string dtype + a (harmless) string-cleanup manifest, where Polars would coerce them. |

**Net: the Polars eviction is complete on the transform + I/O surface.** A default-no-polars install (`goldenflow[native]`, no `[polars]`) reads CSV/Parquet/Excel/DB, runs all 113 transforms, and does dict + file zero-config — all Polars-free. `[polars]` remains only for `transform_df(pl.DataFrame)` (tautological — you need Polars to hold a `pl.DataFrame`) and the optional bulk vectorized speedups. Nothing lingering.
