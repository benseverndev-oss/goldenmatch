...
| **DB I/O** | **remaining** — Polars-free-able | connectorx is Polars-specific; a stdlib-DBAPI→dict reader is Polars-free. Not yet wired (niche). |
| **CSV zero-config** | **remaining** — thesis-fork | `transform("<x>.csv", config=None)` needs dtype inference; `pl.read_csv`'s is finicky and arguably *wrong* for a data-cleaning tool (drops leading zeros `01234→1234`, errors on bigint). Owning a sensible inference (don't coerce IDs/zips) is the thesis-correct move but diverges from `pl.read_csv` — a dates-style "own it" decision. |

Net: after Parquet + Excel closed, the only things a default-no-polars user can't do are **DB read** and **CSV zero-config** — both legitimately optional / thesis-fork, declining loudly with an install hint. CSV/Parquet/Excel read + all transforms + dict zero-config are Polars-free.
