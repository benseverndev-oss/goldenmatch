...
## Packaging addendum (2026-07-15)

Review of the draft PR (#1704) kept the design above but rejected the separate
`golden-diagnostics` package: a new required release surface was not justified
by a single consumer. The report primitives now live in-tree as
`goldenmatch/core/_diagnostics_report.py` (same public functions, same
never-raises / warn-once / `GOLDEN_DIAGNOSTICS=0` contracts), and
`goldenmatch/core/diagnostics.py` imports it directly -- the ImportError guard
in the binding is gone because an in-tree module cannot be missing. The
"Release sequencing" section above is therefore moot: the feature is active on
every plain `pip install goldenmatch`.

**Extraction trigger:** when a second suite member (goldencheck / goldenflow /
goldenpipe / infermap) wants these prompts, extract `_diagnostics_report.py`
back out to a shared `golden-diagnostics` package (publish first, then depend)
