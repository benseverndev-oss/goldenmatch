...
- **Update the stale docstring** at `_build_clusters_via_frames` (~:644-648) which says it computes confidence via `compute_cluster_confidence` "to sidestep re-aligning the kernel's raw cluster_id labels" — the native path now READS `frames.metadata` by `cluster_id` (alignment is direct: kernel + Python both sort-by-min, enumerate start=1). Rewrite that docstring paragraph.

- [ ] **Step 4: `_finalize_clusters` — per-oversized materialization + weak from passed stats**

Add keyword-only `raw_pairs=None` and `weak_stats=None`. In the auto-split loop: when `raw_pairs is not None` AND `cinfo["pair_scores"]` is empty AND the cluster is oversized, materialize it FIRST (before `edge_work += len(...)`): `member_set = set(cinfo["members"]); cinfo_ps = {}; for a,b,s in raw_pairs: if a in member_set and b in member_set: cinfo_ps[(a,b)] = s` (pairs-input order, last-wins — byte-identical to the dict path's fill), assign `cinfo["pair_scores"] = cinfo_ps`. Then the existing `edge_work`/`split_oversized_cluster` works unchanged. AFTER the split loop (columnar path), RESET every cluster's `pair_scores` to `{}` (so split sub-clusters don't retain scores) — gate this on `raw_pairs is not None` so the dict path is untouched. In the weak-quality step (`:604-610`): when `weak_stats is not None`, branch as `if cinfo.get("_was_split"): "split"` (unchanged) `elif cinfo["size"] > 1 and cid in weak_stats: min_edge, avg_edge = weak_stats[cid]; if avg_edge - min_edge > weak_cluster_threshold: weak + confidence*=0.7` `else: "strong"` — i.e. `cid in weak_stats` (absent = no edges = strong, NO bare KeyError); else (`weak_stats is None`) today's `cinfo["pair_scores"]` path verbatim. **CRITICAL:** the dict path (`weak_stats=None`, `raw_pairs=None`) must be UNCHANGED.
