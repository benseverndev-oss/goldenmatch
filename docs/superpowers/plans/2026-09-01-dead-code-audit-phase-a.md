...
def test_no_candidate_is_registry_live():
    """The whole point of the inversion. A registry-reachable module must never
    be reported however few static references it has."""
    live = live_modules()
    assert not {c["module"] for c in candidates(None)} & live


def test_no_candidate_is_allowlisted():
    allowed = load_allowlist()
    assert not {c["module"] for c in candidates(None)} & allowed


def test_every_candidate_carries_its_evidence():
    for c in candidates(None):
        assert c["static"] is True
        assert c["runtime"] in (True, None)


def test_without_coverage_runtime_evidence_is_absent_not_assumed():
    """With no coverage.xml the runtime signal is unknown. It must report None,
    never True -- an unknown treated as proof is how live code gets deleted."""
    assert all(c["runtime"] is None for c in candidates(None))
```
...
In `candidates`, temporarily change `static = unimported_modules() - live - allowed` to `static = unimported_modules() - allowed`. Re-run. Expected: `test_no_candidate_is_registry_live` FAILS. Restore.
