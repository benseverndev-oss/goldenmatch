...
- [ ] **Step 3: Implement**
  ...
  - **Namespace every fixture + truth path by shape AND generate the right shape**: `fixtures / f"bench_{shape}_{rows}.parquet"` (and `.truth.parquet`). The existing `generate()` helper (`orchestrate.py:76-89`) hardcodes `bench_{rows}`, *reuses if it exists*, and spawns `generate_fixture.py` with NO `--shape`. Update it to (a) take `shape`, (b) build the shape-keyed path, AND (c) **append `--shape {shape}` to the `generate_fixture.py` subprocess argv** (Task 3 defaults the generator to `person`). Without (c), `bench_biblio_{rows}.parquet` gets person-shaped columns and every biblio GM lane silently references non-existent `title/authors/venue/year` — a wrong-data run, not a crash. The path-namespacing and the `--shape` arg must land together.
