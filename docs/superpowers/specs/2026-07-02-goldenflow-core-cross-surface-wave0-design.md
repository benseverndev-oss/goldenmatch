...
```python
# reference-mode: Rust is the reference; native runs wherever a WIRED kernel symbol exists
# and the component is not explicitly held on the lossy/opt-out fallback.
...
- `_GATED_ON` the allow-list is replaced by `_has_symbol` over a `_COMPONENT_SYMBOLS` map
  (floor-symbol probe, wheel-skew safe — same pattern as goldenmatch #1343).
- Phone stays correct because its wired symbols exist and its NANP canonical-acceptance guard in
  `_native.py` is unchanged — the guard, not the allow-list, is what keeps phone byte-faithful.
- **`phone_validate` is `_FALLBACK_ONLY`, NOT symbol-absent.** The native kernel *does* export a
  `phone_valid_arrow` symbol (`native-flow/src/lib.rs`), but it calls `phonenumber::is_valid`,
  which implements a *different* validity spec than the product-chosen `is_possible`
  (`is_possible_number`) semantics. It is deliberately **unwired** (there is no
  `phone_valid_native()` in `_native.py`) AND must be listed in `_FALLBACK_ONLY` so the
  reference-mode `_COMPONENT_SYMBOLS` map cannot accidentally route it to native. This mirrors
  goldenmatch's `_FALLBACK_ONLY={sail_scoring}` (a component whose only native path is
  intentionally non-authoritative). This is the honest reason phone_validate stays Python — the
  symbol exists but implements the wrong spec — not "no kernel exists."
- `phone_digits` is genuinely kernel-less (pure Polars) and simply has no `_COMPONENT_SYMBOLS`
  entry.
- The current loader docstring (`_native_loader.py`) carries a stale claim that the `phonenumber`
  crate "exposes no `is_possible_number`"; the kernel's `phone_valid_arrow`/`is_valid` contradicts
  the framing. The Wave 0 rewrite corrects this comment so the reference-mode loader does not
  inherit the confusion — the accurate statement is "`is_valid` exists but is not the chosen
  validity spec, so phone_validate stays Python."
...
(copied into the TS package's test fixtures with a CI sync-check that fails on drift — **copy, not
symlink**, since Windows dev has no symlinks without Developer Mode per root CLAUDE.md). Each line: `{"transform": "...", "input": "...", "expected": "..."}`
