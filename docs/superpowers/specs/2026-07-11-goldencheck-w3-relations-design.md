...
Base: fresh `origin/main` at branch time (W0-land + CSV + W1 merged; W2 #1681 enqueuing, NOT yet in this worktree's base — so cite `aggregate.rs` (W1) + `keys.rs` (multi-column precedent) as the current pattern, NOT the W2 stats/sequence/freshness files). Rebase onto fresh origin/main before PR (W2 will have merged by then). Kernels present at base: benford/keys/composite/FD/approx-FD/fuzzy/regex/str_to_date/csv_infer/column_aggregate.
...
- **[B/BLOCKER] Do NOT reuse `intern_column` for the cast.** Intern assigns null a distinct reserved id, but the profiler does `cast(Utf8).fill_null("")` — so **null and an actual `""` MUST collide** (a col `[null, ""]` is ONE exact group in Polars). The kernel builds REAL signature strings: per cell, `fill_null("")` FIRST, then (for string cols) `to_lowercase -> replace_all(r"[^0-9a-z]+"," ") -> trim` — so `"!!!"` and null and `""` all normalize to `""` and collide, matching approx_duplicate.py. Order: fill_null → lowercase → replace → trim.
- **Signature = count-invariance holds; only NON-injective casts matter.** Int/finite-float (shortest round-trip, both sides)/Date32/Bool casts are injective → automatically partition-equal to Polars regardless of exact string form (so DON'T sweat bool casing / date ISO form — not load-bearing). The only non-injective cases: null-vs-`""` (handled above) and `NaN` (Rust `Display` `f64::NAN`→"NaN" collapses all NaN bit patterns to one group, MATCHING Polars' one "NaN" group). So the **"cast-collision" divergence class is likely EMPTY** — do not pre-register it; only add a divergence if a fixture actually shows one.
- **[S] `is_string` mask comes from PYTHON, not Arrow-sniffing.** The profiler normalizes only where `dtype == pl.Utf8` (approx_duplicate.py:39); a Polars Categorical/Enum is NOT pl.Utf8 (un-normalized) but `to_arrow()` emits `Dictionary(_,Utf8)`. Pass an explicit `is_string: Vec<bool>` = `[dt == pl.Utf8 for dt in df.dtypes]` into the kernel; NEVER infer "string" from `array.data_type()`.
- **[S] Multi-array FFI is a SOLVED pattern** (downgrade the risk): `keys.rs` `composite_key`/FD already take `Vec<PyArrowType<ArrayData>>` in the shim → `to_arrays()` → `&[ArrayRef]` in core. **Signature:** shim `duplicate_signatures(field_arrays: Vec<PyArrowType<ArrayData>>, is_string: Vec<bool>) -> DupStats`; core `duplicate_signatures(columns: &[ArrayRef], is_string: &[bool])`. Wiring passes per-Series `df[c].to_arrow()` (single Array each), NOT `df.to_arrow()` chunked columns.
- **[S] harness divergence granularity is per-COMPONENT, not per-fixture.** `ACCEPTED_DIVERGENCES` matches on `component == name`, folding all a component's fixtures into one pass. If a float-edge divergence ever needs registering, put those fixtures under a SEPARATE component name (e.g. `duplicate_signatures_floatedge`) so the main `duplicate_signatures` component stays exact-required (else a real int/string partition bug is silently suppressed). Default: keep it one exact component (per above, no divergence expected).
- **normalize lowercase = Rust std `str::to_lowercase`** (Unicode-aware, matches Polars which is Rust-backed) — do NOT hand-roll ASCII lowercase. `strip_chars()` no-arg is NOT load-bearing (the regex already collapsed every non-`[0-9a-z]` run incl. Unicode whitespace to one ASCII space). `str_replace_all` (regex.rs) and Polars `str.replace_all` are both the Rust `regex` crate = byte-identical. Add ONE Unicode-lowercase fixture ("İ"/"Σ") to catch any residual.
- **[age] `age_mismatch` is bit-exact → register EMPTY.** `(ref_epoch_days - dob_epoch_days) as f64 / 365.25` is the identical f64 op as Polars `(reference_date - dob).dt.total_days()/365.25`; `|actual-expected| > 2.0 & both-non-null` matches. `sample_indices` (first-5 mismatch, ROW order) → the caller's `col_series.filter(mask).head(5)` gathers the same rows in the same order (filter is order-preserving). Pin the DOB parse in the shadow to match `dob_expr` (str.to_date `%Y-%m-%d` strict=False, or `cast(Date, strict=False)` per original dtype — approx age_validation.py:106-109). Add a NaN-age fixture (`NaN > 2.0` is False both sides → excluded).
