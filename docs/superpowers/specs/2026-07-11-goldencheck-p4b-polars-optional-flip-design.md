...
2. **Clean decline in ONE place** (`_polars_lazy.py`): when the lazy proxy's first `import polars` fails, raise a helpful `ImportError` ("This GoldenCheck operation needs Polars; install `goldencheck[polars]`. Parquet/Excel reading and `scan_columns` work without it.") instead of the raw `ModuleNotFoundError`. Because EVERY polars-touching call site reaches Polars through `from goldencheck._polars_lazy import pl`, this single change gives every polars-required path (`scan_dataframe`, `scan_file`, baseline, drift, CLI scan, `_read_csv_columns`, the R4-declined relation profilers) a clear, actionable error when polars is absent — no per-call-site wrapping needed.
