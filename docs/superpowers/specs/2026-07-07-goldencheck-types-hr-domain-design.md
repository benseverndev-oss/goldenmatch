...
**Use HR-DISCRIMINATIVE hints, deliberately excluding generic person tokens** (`first_name`, `last_name`, `email`, `phone`, `name`, `address`). Rationale: broad person tokens would score high on the employee data but **false-positive** — any customer/person dataset (`first_name, last_name, email, phone`) would mislabel as `hr`. Discriminative tokens (`employee_id`, `salary`, `department`, `hire_date`, `employment_status`, `manager`, `performance`, `pay_grade`, `job_title`, `remote_work`, `tenure`) fire on HR data and score ~0 on non-HR person data, and cannot steal finance/healthcare/ecommerce (their columns don't match HR hints).

**Verified on the box** (`_detect_core_pure`, the real kernel) with the hint set below:

| Dataset | Detect result |
|---|---|
| Target employee (12 col) | `hr` **0.58**, confident |
| Employee + 1 extra non-HR col (13) | `hr` **0.54**, confident (robust to column count) |
| Typical HR export (`emp_id, designation, department, date_of_joining, salary, manager, employment_status`) | `hr` **0.75** |
| Generic person (`first_name, last_name, email, phone, city, state`) | **None** (no false positive) |
| Customer (`customer_id, name, email, phone, address, status`) | **None** (hr only 0.17 from lone `status`) |
| Finance | `finance` 0.6, hr **0.0** |
| Healthcare | `healthcare` 1.0, hr **0.0** |

The lone generic-ish token is `status` (employment status); it contributes at most 1/n to a non-HR dataset — far below threshold on its own.
...
Types (the union of `name_hints` is what detect scores — the verified discriminative set, organized into types):
...
- **salary**: `[salary, compensation, wage, annual_salary, base_salary, ctc]` — numeric.
- **pay_grade**: `[pay_grade, grade, band, job_level]` — low-cardinality.
- **hire_date**: `[hire_date, join_date, joining_date, start_date, date_of_joining, doj]`
- **employment_status**: `[employment_status, employee_status, status, employment_type]` — low-cardinality.
- **manager**: `[manager, supervisor, reports_to, reporting_manager]`
- **performance_rating**: `[performance, performance_score, appraisal, rating]` — low-cardinality.
- **work_arrangement**: `[remote_work, work_mode, work_location]`
- **tenure**: `[tenure, years_of_service, experience]` — numeric.

At implementation, the FINAL `hr.yaml` is validated by the real load path (write it, run `detect_domain_detailed` on the employee df → assert `domain == "hr"`, `score ≥ 0.5`); if the type-organized token set shifts the score, adjust to keep the §2 verified outcomes.
...
- **Python detect test** (box-runnable) — `packages/python/infermap/tests/test_hr_domain.py` (or extend `test_dictionaries.py`): assert `available_domains()` includes `"hr"`; assert `detect_domain_detailed(df)` on an employee-column df returns `domain == "hr"` with `score >= 0.5`; assert a generic person df does NOT detect `hr`. Uses the real shipped pack via goldencheck-types.
