...
retriage.md` -- the coverage-based fix this document calls for is now built
and run against the full population (53 high-confidence findings as of that
run). The "worth enforcing" list below is closed: all five priority items
(`_alias_score_matrix`, all four `core/survivorship/native.py` claims --
one more than counted here, `canonical_soundex`) are confirmed enforced;
`simhash_band_hashes` and `_do_transform_columnar`'s pattern-claim
reclassification held under real coverage data. The "still unmeasured" false
-negative rate this document leaves open is now measured, with a real
false-positive class this document did not anticipate. Read both.
**Date:** 2026-09-02
