...
- **Semantic-layer catalog emitters (MetricFlow / Cube / OSI) +
  `emitSemanticModelFromStore` + the `emit_semantic_model_from_store` MCP tool.**
  Ports the previously-Python-only semantic catalog subsystem: a stats-only
  `ResolvedCrosswalk`, the three dialect emitters (`emitFromCrosswalk` /
  `emitCubeFromCrosswalk` / `emitOsiFromCrosswalk`), and a
  PyYAML-`safe_dump`-compatible block-YAML serializer (`dumpYaml` in
  `core/semantic/yamlEmit.ts`, with a `PyFloat` wrapper so `reduction_ratio`
  emits `0.0`/`0.2` bare). `emitSemanticModelFromStore(store, opts)` reads the
  live control plane (`identitySummaryStats`) and regenerates the conformed
  `resolved_entity_id` join declaration on demand — "keep the semantic layer's
  identity join live against the control plane." Faithful port of Python
  `semantic/{crosswalk,metricflow,cube,osi,catalog}.py`; a committed
  cross-language fixture (`tests/parity/fixtures/semantic/catalog-emit.json`,
  generated by `scripts/emit_semantic_catalog_fixtures.py` and wired into
  `regen_ts_parity_fixtures.sh`) locks the emitted YAML **byte-for-byte** against
  Python `yaml.safe_dump`. Exposed as the TS MCP `emit_semantic_model_from_store`
  tool (identity tools 17 → 18, total MCP 85 → 86), flipping it from
  `python_only` to `shared` under `mcp_tools`; auto-advertised on the TS A2A card,
  so it is declared `a2a_skills.ts_only`. This reverses the "stays Python-only"
  note in the `certify_serving_joins` entry above. New surface exports from
  `goldenmatch/core` via `core/semantic`.
- **Semantic-model certifier (consume side, `certifySemanticModel`) + the
  `certify_semantic_model` MCP tool.** The symmetric complement of the catalog
  emitters: parse a dbt/MetricFlow, Cube, or OSI/Ossie model (dialect
  auto-detected) and certify every key its metrics join on via the structural
  key-integrity tier — is it unique at grain, and how much would a duplicated key
  inflate a SUM/COUNT (fan-out)? Ports Python `semantic/certify.py::certify_semantic_model`
  plus the consume halves of the three dialect modules: `parseSemanticModels`
  (MetricFlow), `parseCubeModels` / `cubeJoinKeys` / `certifyCubeJoins` (Cube),
  `parseOsiModels` / `osiJoinKeys` / `certifyOsiRelationships` (OSI), and
  `detectDialect`. Edge-safe: the core operates on an already-loaded document
  object (a plain JS object) + column-oriented frames; the YAML/CSV file reads
  live in the node MCP handler (via the optional `yaml` peer). The Python
  `resolve=true` entity-resolution fragmentation/undercount tier stays Python-only
  (needs the dedupe pipeline), matching the structural-only `certifyKeyIntegrity`.
  A committed cross-language fixture (`tests/parity/fixtures/semantic/certify.json`,
  generated by `scripts/emit_semantic_certify_fixtures.py` — several cases feed the
  emitters' own output back through the certifier, so `parse(emit(...))`
  round-trips — wired into `regen_ts_parity_fixtures.sh`) locks the TS
  certification against Python across all three dialects. Exposed as the TS MCP
  `certify_semantic_model` tool (total MCP 86 → 87), flipping it `python_only →
