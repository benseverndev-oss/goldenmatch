...
- **Blocking strategies `lsh` + `perceptual` (2026-07-25):** ported to TS so both move `python_only → shared`. `blocker.ts` gained `buildLshBlocks` (over the already-ported `MinHashLSHBlocker`/`sketch.ts`, `sketch-core` kernel) + `buildPerceptualBlocks` (pure banded-hamming LSH over hex pHashes, BigInt band math; `perceptual-core` kernel) + dispatch cases; `LSHKeyConfig`/`PerceptualKeyConfig` interfaces + `BlockingConfig.strategy` union + `VALID_BLOCKING_STRATEGIES`. Cross-language fixture `tests/parity/fixtures/blocking-strategies.json` (emitted by `emit_blocking_parity_fixtures.py`, wired into `regen_ts_parity_fixtures.sh`) — `blocking-strategies.test.ts` asserts identical block membership (value-based by `id`). `simhash` stays python_only (model-backed: needs per-record embeddings, no bundled edge model — the `retrieve_similar` boundary); it's classified in the new `blocking_kernels_deferred` map gated by `check_blocking_coverage`.
- **FS domain comparators `date_diff` / `geo_haversine` (score_one ids 17/18, 2026-07-25 Phase 4):** now single-sourced from the score-core kernel on every runtime. Pure-TS `dateDiffSimilarity` (Hinnant day-ordinal bands + MM/DD transposition floor) / `geoHaversineSimilarity` (haversine km bands) in `core/scorer.ts` + `SCORER_ID` 17/18 in `wasm/backend.ts` (so the score-wasm `score_matrix` catch-all accelerates them — no Rust change) + `VALID_SCORERS`. Oracle fixture `tests/parity/fixtures/scorer-domain-comparators.json` (Python-reference == Rust); pure-TS==reference in `scorer-domain-comparators.test.ts`, wasm==pure-TS in `wasm-scorer.test.ts`. Both moved to `scorers`/`scorer_kernels` **shared** in `parity/goldenmatch.yaml`.
- **`array_intersect` (score_one id 19, jaccard default, 2026-07-25):** set-overlap over delimited strings (Splink ArrayIntersect → GM), now kernel-single-sourced too. Pure-TS `arrayIntersectSimilarity` in `core/scorer.ts` (token-set Jaccard / overlap-coefficient, exact-string fallback on empty set) + a `startsWith("array_intersect")` guard BEFORE the `scoreField` switch (the mode `:overlap` rides the name, so an exact-match `case` can't express it) + `SCORER_ID` 19 (**bare form only** → WASM covers jaccard-default; `:overlap` stays pure-TS) + `VALID_SCORERS`. Same `scorer-domain-comparators.json` oracle. **`array_intersect:overlap` is deliberately NOT WASM-covered** — the fixed-id `score_matrix` can't carry the mode, mirroring the Python native decline; it's pure-TS (covered by `scorer-domain-comparators.test.ts`).
- **`numeric_diff` (score_one id 22, pct:0.1 default, 2026-07-25 Part D):** the last scorer, now kernel-single-sourced too. Pure-TS `numericDiffSimilarity` in `core/scorer.ts` (abs/pct banded ramp, exact-string fallback on unparseable) + a `startsWith("numeric_diff")` guard before the `scoreField` switch + `SCORER_ID` 22 (**bare pct:0.1 form only** → WASM covers the default; `numeric_diff:abs|pct:<band>` ride the scorer string and stay pure-TS) + `VALID_SCORERS`. Same `scorer-domain-comparators.json` oracle. **Arc complete: no scorer remains `python_only`; `scorer_kernels_deferred` holds only the two model-backed embedding scorers.**
- **`cosine` (score_one id 23, 2026-07-26 Part E):** vector cosine over two PRECOMPUTED float-vector columns (Splink `array_cosine_similarity` → GM) — the compute-closure arc's final scorer. Pure-TS **`vectorCosineSimilarity`** in `core/scorer.ts` (named to avoid the embedder `cosineSimilarity(number[],number[])` collision — this one PARSES delimited vector strings) + a plain `case "cosine"` in `scoreField` (no mode, so no prefix guard) + `SCORER_ID` 23 (→ WASM catch-all covers it) + `VALID_SCORERS`. Same `scorer-domain-comparators.json` oracle (14 cosine cases); `wasm-scorer.test.ts` pins wasm==pure-TS to 12dp (correctly-rounded sqrt + identical accumulation order). Moved to `scorers`/`scorer_kernels` **shared**. **No scorer or blocking strategy remains single-language across the suite.**
...
- **Agent `selectStrategy` PPRL gate (2026-07-25, parity fix):** `selectStrategy(profile, allowPprl = false)` now mirrors Python `select_strategy(allow_pprl=)` — sensitive fields route to `pprl` ONLY when opted in; otherwise the normal tree runs and `pprl_available: true` flags PPRL as available (new `StrategyDecision.pprl_available` field). Previously TS returned `pprl` unconditionally on `has_sensitive` — a real Python↔TS divergence that sat undetected because `emit_agent_fixtures.py` was never in `regen_ts_parity_fixtures.sh` (the #856 hole). Now wired in (alongside `emit_anomaly_fixture` + `emit_scorer_parity_fixtures`), so `agent-decisions.json` drift is caught by `ts_parity_freshness`. Fixture regenerated; `agent-strategy.test.ts` asserts the corrected decision.
