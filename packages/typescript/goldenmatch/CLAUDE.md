...
- **Agent tools `sensitivity` / `incremental`** — the agent-level tools whose Python
  implementations (`run_sensitivity` / `run_incremental`) have no TS core. Porting them
  is a separate effort; for now they are Python-only and **NOT advertised** on the TS
  MCP/A2A surface (no silent gap — the card and tool list expose only what is wired).
  **`certify_recall` is NO LONGER Python-only** — it was ported to TS on 2026-07-23
  (Tier 3 PR-3, `src/core/recall-certificate.ts`) and is now a `shared` base MCP tool;
  see the MCP-tool-count note below. (This reverses the 2026-06-15 "declared, not a gap"
  Python-only decision for `certify_recall` specifically.)
