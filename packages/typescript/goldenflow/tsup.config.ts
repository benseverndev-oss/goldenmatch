...
export default defineConfig({
  ...
  splitting: false,
  treeshake: true,
  // Copy the opt-in WASM artifact (built by goldenflow-wasm/build_wasm.sh) into
  // dist so the loader's `new URL('./artifacts/goldenflow_wasm_bg.wasm',
  ...
  // (goldenflow_wasm.js) and the .wasm artifact stay external (see `external`).
  ...
  external: [
    ...
    // esbuild never tries to resolve `./artifacts/goldenflow_wasm.js` at build
