"""GoldenMatch binding of the shared ``golden-diagnostics`` issue-reporter.

Pins package / version / repo so call sites read one helper, and is **guarded**
so diagnostics is never load-bearing: if ``golden-diagnostics`` is unavailable
(or errors), every helper is a silent no-op and GoldenMatch runs unchanged.
...
try:
    import golden_diagnostics.report as _gd

    _HAS_DIAG = True
except Exception:  # noqa: BLE001 - diagnostics must never break an import
    _gd = None  # type: ignore[assignment]
    _HAS_DIAG = False
...
def report_anomaly(category: str, summary: str, **kwargs: Any) -> None:
    """Report a GoldenMatch anomaly (no-op if diagnostics is unavailable)."""
    if not _HAS_DIAG:
        return
