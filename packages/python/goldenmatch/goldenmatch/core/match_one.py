...
def match_one(
    ...
    Args:
        ...
        store: Optional ``CandidateStore`` for ANN candidate retrieval (e.g.
            ``LanceCandidateStore`` for an out-of-core base). When ``None``, an
            in-memory ``FrameCandidateStore`` wraps ``df`` — byte-identical to the
            legacy path.
    ...
    if ann_blocker is not None and embedder is not None and ann_column is not None:
        return _match_one_ann(record, df, mk, ann_blocker, embedder, ann_column, top_k, store)
...
def _match_one_ann(
    ...
    from goldenmatch.core.candidate_store import FrameCandidateStore
    ...
    # Resolve candidate rows via the store — gathers only the top-K rows, not the
    # whole frame. ann_score is intentionally unused (parity with legacy).
    if store is None:
        store = FrameCandidateStore(df)
    positions = [faiss_idx for faiss_idx, _ann_score in candidates]
    rows, row_ids = store.take(positions)

    results = []
    for candidate_row, candidate_id in zip(rows, row_ids):
