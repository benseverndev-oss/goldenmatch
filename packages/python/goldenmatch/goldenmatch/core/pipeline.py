...
def _em_excluded_fields(blocking) -> list[str]:
    """Fields to exclude from Fellegi-Sunter EM: those agree-by-construction in
    EVERY candidate-generation pass (the INTERSECTION of pass field-sets). A field
    used by only SOME passes still varies in pairs from the other passes, so EM can
    estimate its m/u -- excluding it would neuter a real signal (it did: union
    exclusion forced names to fixed [-3,0,3] and halved recall on historical_50k).
    Single static key -> that key (always agrees, exclude). Multi-pass union ->
    usually empty (train all fields).

    A pass is a candidate-generation unit: ``blocking.passes`` when present, else
    each ``blocking.keys`` entry as its own pass. A field is agree-by-construction
    only within the pairs generated by passes that include it; under multi-pass
    UNION blocking the same field varies in pairs from other passes, so the only
    fields safe to exclude are those present in EVERY pass.
    """
    if blocking is None:
        return []
    passes = blocking.passes if blocking.passes else (blocking.keys or [])
    field_sets = [set(p.fields) for p in passes if p.fields]
    if not field_sets:
        return []
    common = set.intersection(*field_sets)
    # deterministic order: first non-empty pass's order, filtered to the intersection
    first = next((p.fields for p in passes if p.fields), [])
    return [f for f in first if f in common]
