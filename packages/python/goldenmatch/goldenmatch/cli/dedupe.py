...
def dedupe_cmd(
    ...
    chunked: bool = typer.Option(False, "--chunked", help="Deprecated no-op: use --backend chunked for large-dataset chunked processing."),
    ...
    diff_html: bool = typer.Option(False, "--diff-html", help="Deprecated no-op: accepted for back-compat but not generated; use --diff for a CSV diff."),
    ...
    anomaly_sensitivity: str = typer.Option("medium", "--anomaly-sensitivity", help="Deprecated no-op: accepted for back-compat but not applied (anomaly sensitivity is fixed)."),
