...
the sdist). `scripts/run_benchmarks.py` auto-pulls them as part of a run; to get
them *without* sitting through a benchmark, use `--download-only`:

```bash
python scripts/run_benchmarks.py --download-only --datasets all       # everything
python scripts/run_benchmarks.py --download-only --datasets dblp-acm  # just one
python scripts/run_benchmarks.py --download-only --datasets products  # Abt-Buy + Amazon-Google
```

Idempotent, so it is safe to re-run. It prints each dataset as `ok`/`MISSING`
and exits non-zero if anything is missing. Each dataset honours a URL override
(`GOLDENMATCH_DBLP_ACM_URL`, `GOLDENMATCH_ABT_BUY_URL`,
`GOLDENMATCH_AMAZON_GOOGLE_URL`, `GOLDENMATCH_NCVR_SAMPLE_URL`) for a mirror.

Two datasets are not pulled from a public URL: **NCVR** is an NC voter roll
extract and needs `GOLDENMATCH_NCVR_SAMPLE_URL` pointed at a hosted 10k sample,
and **Febrl3** is generated by `recordlinkage` rather than downloaded. Benchmark
tests needing an absent dataset **skip** with a message naming the command to
get it -- they do not fail.
