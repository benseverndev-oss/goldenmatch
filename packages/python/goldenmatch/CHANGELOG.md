...
### Added
- **`MatchkeyConfig.link_threshold_observed` -- the FS link cutoff a run resolved, recorded without being chosen.** Closes the second half of #2637. `link_threshold` carried two meanings at once: the *value*, and the assertion "a human chose this, do not refit" -- `resolve_thresholds` short-circuits on it. That is why stamping a resolved **fallback** onto it was not behaviour-neutral and cost pairwise precision **0.9308 vs 0.9992** at person@100K, and why the original fix was narrowed to `calibrated`-only, leaving fallback runs (the common case, since `GOLDENMATCH_FS_CALIBRATE_THRESHOLD` is off by default) exactly as inert as before the issue was opened: `ThresholdShift` returned `None`, `threshold_perturbations` returned `[]`, and `perturbation_stability` was permanently unmeasurable on the FS path -- verified structural on 4 of 4 datasets in #2636. Splitting provenance from value is smaller than the "third resolver state" that follow-up proposed: the new field is a **record**, `resolve_thresholds` never reads it, so writing it cannot pin a cutoff or skip a refit. Perturbation now anchors on `MatchkeyConfig.perturbation_basis` (`cutoff` if something chose one, else the observed value); `cutoff` itself is deliberately unchanged and still answers `None` when nothing decided, or the fabricated-default problem of #2483 returns. Measured on a fallback config: perturbations 0 -> 2, `ThresholdShift` inert -> applies, and `link_threshold` stays `None` so the full-data refit still runs. Safe on both consumers: `_perturbation_stability` discards its variants after reading their profiles, and `optimize_config` is opt-in and not on the controller's commit path. (#2637)


### Added
- **`segments.infer_kinds` fills a segment's `kind` from VALUES, and abstains unless the evidence is unambiguous.** `kind` -- person / organization / asset / place -- is the axis downstream matching keys off, and it arrives populated only when the detected role appears in some domain pack's vocabulary. #2574 Part B measured every other layer at `kind="unknown"`: 331 of 608 columns across all three layer corpora, each in a group that was correctly formed and correctly self-named, missing only the one field consumers read. No name yields `film -> asset` without enumerating every noun in every vertical, which is the losing game the closed `IDENTITY_KINDS` set exists to avoid; values do yield it. **Measured on the real frames this repo has**, per-token surname rate over value-classified name-like columns: NCVR `last_name` 0.696 / `middle_name` 0.587 / `first_name` 0.423 at 1.0 tokens per value, against product and paper titles at 0.050-0.122 and 4.5-6.7 tokens per value -- a ~3.5x gap. Two guards earn their place: a tokens-per-value ceiling, because a per-VALUE metric scores product *descriptions* at 0.83-0.86 (a 10k-surname list contains Brown, Long, Case, Price, and long text hits one almost surely), and a USPS street-type exclusion, because street names ARE surnames and NCVR's address column scores 0.266. **Only `person` is inferred**, and abstention is the design rather than a limitation of it: the corpus available here is one real person frame, six asset frames and no organization or place frame at all, so a classifier fitted to more than that would be the self-agreement failure the layer corpora were built to escape. Abstaining costs nothing -- it is the status quo -- and the only risk a label can carry is being wrong. Verified zero false positives across all six asset frames plus two non-name person-frame columns. Reuses the shipped `refdata` vocabularies (census surnames, USPS street types) rather than adding any; the checksum-bearing identifier vocabulary #2574 sized for this turned out to be unnecessary. Placed in `goldenmatch.core.segments` rather than InferMap because that is where the vocabularies live and where `kind` is consumed -- InferMap cannot depend on GoldenMatch. Deliberately inert with respect to matching, like the rest of that module: it labels. (#2758)
