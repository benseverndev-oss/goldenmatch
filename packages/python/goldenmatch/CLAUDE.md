...
- **Best-effort commit when iterations don't reach green** (intended improvement; see notes below). `RunHistory.pick_committed()` returns the highest-ranked entry by lex key `(health_rank, -mass_separation, iteration)`. Replaces `cheapest_healthy()` which is now a deprecation alias (removed in v2.0). DeprecationWarning text calls out the behavior change explicitly.
- **`RunHistory.stop_reason: StopReason | None`** populated at every break point in the controller's iteration loop. Observable via `result.postflight_report.controller_history.stop_reason`. `StopReason` enum lives in `core/complexity_profile.py` (moved from controller).
- **Virtual v0 entry + precision-collapse floor.** Phase 5.2 surfaced a regression where committing a RED iter-N config could be substantially worse than v0 (DQbench T1 collapsed to 1% precision). Fix: append `config_v0`'s profile as a synthetic `HistoryEntry(iteration=-1)` to history before `pick_committed()`, plus `pick_committed(precision_collapse_floor=0.9)` demotes RED entries with `mass_above_threshold > 0.9` to rank=3. Restored v1.8 parity exactly. WARNING/INFO log uses `iter=v0` to identify virtual-entry commits.
- **Health-aware commit logging.** WARNING when committed health is RED (with failing sub-profile name + stop_reason); INFO when YELLOW; silent on GREEN. ERROR when every iteration errored (falls back to v0 + RED sentinel — preserved from v1.8).
- **Spec premise correction (recorded for v1.10).** Original v1.9 spec assumed best-effort RED commit would let post-iteration plumbing run on tiers where iteration can't reach YELLOW. In practice, on DQbench T1/T2 the committed RED config is *worse* than v0 because the controller's complexity indicators can't distinguish "blocking key is wrong" from "blocking key is right but sample has no visible matches" — both produce `mass_above_threshold=0.0`. Virtual-v0 + floor protects against the regression but doesn't add gain on these tiers. v1.10 will add new indicators (identity-column priors, cross-blocking overlap probe, blocking-column corruption signal) so the controller can tell these cases apart.
