...
- **Splink-Spark roadmap Phase 3 — distributed clustering (2026-05-19, branch `feat/phase3-distributed-clustering`):** label propagation on Ray Datasets replaces driver-side `UnionFind` for the scored-pair clustering stage. `goldenmatch.distributed.clustering` exports `pairs_list_to_dataset`, `label_propagation`, `build_clusters_distributed`, `materialize_cluster_dict`, `ConvergenceError`. `core.cluster.build_clusters` polymorphic on `list[tuple] | ray.data.Dataset` via `is_ray_dataset` dispatch — in-memory callers unchanged (additive change). Bounded at 30 iterations; falls back to driver-side scipy.csgraph on non-convergence with a WARNING log. Adapter `materialize_cluster_dict` collects per-cluster aggregates back to the existing `dict[int, dict]` shape (Phase 4 removes the adapter). `_finalize` pipeline cheat-line in `goldenmatch.distributed.pipeline.run_dedupe_pipeline_distributed` still materializes for the golden stage; Phase 4 removes that. Kill criterion: cluster wall < 60s at 25M pairs (vs current 126.6s single-node UnionFind, run `26095134836`). `scripts/bench_phase3_cluster.py` + `bench-phase3-cluster` workflow job. Bench requires a pre-scored `pairs_<rows>.parquet` on the release. Spec: `docs/superpowers/specs/2026-05-19-phase-3-distributed-clustering-design.md`. Plan: `docs/superpowers/plans/2026-05-19-phase-3-distributed-clustering.md`.
