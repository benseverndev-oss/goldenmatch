...
- **Recommended for 5M today:** `backend="chunked"` + `config_mode="explicit-personlike"` (or any explicit blocking config — controller's 1K-sample blocking discovery can fail with sparse duplicates at large N). The pure zero-config path with chunked is a v2 follow-up.
- **Scale-aware backend auto-selection (2026-05-15):** `auto_configure_df` now sets `config.backend="duckdb"` automatically when `df.height >= 1_000_000` so the zero-config path stops OOMing at 5M on 16 GB. Helper: `_scale_aware_backend(row_count)` in `core/autoconfig.py`. Env overrides: `GOLDENMATCH_AUTOCONFIG_BACKEND=0` disables, `=duckdb`/`=ray` forces, `GOLDENMATCH_AUTOCONFIG_BACKEND_THRESHOLD=<int>` adjusts the cutoff. Explicit user-set `config.backend` is preserved (the helper only fires when the field is still `None` post-controller). Tests: `tests/test_autoconfig_backend_routing.py`.
