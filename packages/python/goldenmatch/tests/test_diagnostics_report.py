"""Tests for the in-tree issue-reporter primitives (``_diagnostics_report``)."""
...
import pytest
from goldenmatch.core._diagnostics_report import (
    environment_report,
    is_expected,
    issue_url,
    report_anomaly,
...
def isolate_diagnostics(monkeypatch: pytest.MonkeyPatch):
...
def test_report_anomaly_fires_once_per_key(caplog: pytest.LogCaptureFixture):
...
def test_kill_switch_silences(monkeypatch: pytest.MonkeyPatch):
...
def test_report_anomaly_never_raises(monkeypatch: pytest.MonkeyPatch):
    # even if logging blows up, report_anomaly swallows it
    class BadLogger:
        def warning(self, *a: Any, **k: Any) -> None:
            raise RuntimeError("logging down")

    assert report_anomaly("x", "y", logger=cast(logging.Logger, BadLogger())) is None
