...
- Optimal 1:1 assignment via `infermap-core::linear_sum_assignment` (Hungarian/LAP; single-sourced 2026-07-26). Python dispatches native→`_lsa_pure` (byte-identical); TS uses `core/assignment/hungarian.ts` (the reference the Rust kernel was ported from). This REPLACED `scipy.optimize.linear_sum_assignment` on the assignment path — the old scipy path disagreed with TS on ties (both optimal, different pick). Cross-language locked by `tests/fixtures/assignment_parity.json` (read directly by `test_assignment.py` + TS `assignment-parity.test.ts`). `linear_sum_assignment` is gated in `_native_loader._GATED_ON`/`_COMPONENT_SYMBOLS`; native==pure in `test_native_parity.py`.
- **scipy fully EVICTED from the runtime** (2026-07-29): the last use was Platt calibration's `scipy.optimize.minimize(method="Nelder-Mead")`, now owned in `infermap/_optimize.py` (`minimize_nelder_mead`, byte-close to scipy on the convex log-loss). `pip install infermap` is scipy-free; `numpy` is now an explicit dep (was transitive via scipy). scipy stays a dev-group parity oracle (`test_optimize_parity.py`).
