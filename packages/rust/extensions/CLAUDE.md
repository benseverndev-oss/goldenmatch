...
- `docs.rs` -- **`goldenmatch_docs() -> TEXT` (0.18.0)**, the in-SQL orientation function. Returns the extension's `llms.txt`, `include_str!`'d into the shared library at build time. A SQL connection is the surface with the least for an agent to read -- no filesystem, no package to import, just ~80 function names -- so this is the pointer to the authoritative docs that every other surface now ships inside its installed artifact. Zero-arg, `IMMUTABLE PARALLEL SAFE`, and free of every dependency the rest of the crate carries (no embedded CPython, no SPI, no GUC, no file I/O), so it answers even in a backend where the Python bridge fails to initialise. `rust_pgrx` smoke-asserts the returned text really carries the pointers -- an empty or truncated `include_str!` would otherwise ship silently. DuckDB has the same function (`goldenmatch_duckdb/docs.py`, reads its own packaged `llms.txt` at call time rather than compile time, since it is a Python package).
- `fs_em.rs` -- native-direct Fellegi-Sunter training from COUNTED comparison vectors (**0.19.0** -- Phase 1c of `docs/superpowers/specs/2026-08-13-fs-em-rust-single-source-design.md`). **`goldenmatch_train_em_from_counts(n_levels int[], patterns int[], counts float8[], u_probs float8[], conditioned bool[], max_iterations int, convergence float8) -> TEXT`** calls the pyo3-free `score-core::em_core` kernel directly -- **no embedded CPython**. `goldenmatch_train_em` still bridges to Python because pair sampling + binning were never ported; this function starts one step later, from vectors the ENGINE counted, which is the half a SQL engine is actually for (`GROUP BY` over the gamma columns, bounded by `prod(levels + 1)` distinct rows). Flat arrays per the `goldenmatch_hnsw_pairs(flat_vecs, dim)` idiom; `u_probs` is RAGGED and split back by `n_levels`. Fail-soft to a `{"error":..}` envelope. **No `#[pg_test]`** -- `cargo pgrx test` cannot run for this crate, so the assertions are a `rust_pgrx` psql smoke pinned to the SAME anchors as `score-core/tests/fixtures/em_counts_parity.json`.
- `key_integrity.rs` -- native-direct structural key-integrity certifier (**0.17.0** — the semantic-layer wedge). **`goldenmatch_certify_structural(input_json TEXT) -> TEXT`** calls the pyo3/pgrx-free `goldenmatch-key-integrity-core` crate directly (the SAME reference kernel the `goldenmatch[native]` wheel's `semantic.certify_structural_json` + the DuckDB `goldenmatch_certify_structural` UDF run — byte-identical across Python/TS/DuckDB/Postgres by construction, shared kernel + shared golden). **JSON in / JSON out** (the core's native contract — measures make a columnar signature unwieldy): in `{n_rows, group_columns:[[..]], measures:[{name,values}]}`, out `{n_rows, n_key_groups, duplicate_key_groups, max_fan_out, is_unique_at_grain, measure_fan_out:{name:ratio}}`. Non-STRICT + fail-soft: invalid input returns a `{"error":..}` envelope (the DuckDB convention), never raising. `rust_pgrx` `#[pg_test]` asserts the pinned duplicated-key certificate shared with the DuckDB + Python surfaces.
...
- `key_integrity_kernels.py` -- `goldenmatch_certify_structural(input_json VARCHAR) -> VARCHAR`, the structural key-integrity certifier (the semantic-layer wedge). JSON-in/JSON-out over `goldenmatch.semantic.certify_structural_json` (the SAME reference the Postgres native `key-integrity-core` surface + the wheel run), so byte-identical to Postgres/Python/TS. Fail-open if `goldenmatch.semantic` isn't importable; fail-soft to a `{"error":..}` envelope on bad input. Registered from `functions.register`. Tests: `tests/test_key_integrity_kernels.py` (pinned + golden-parity + matches-reference).
