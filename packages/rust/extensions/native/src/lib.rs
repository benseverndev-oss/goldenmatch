...
fn _native(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(score::array_intersect_similarity, m)?)?;
    m.add_function(wrap_pyfunction!(score::numeric_diff_similarity, m)?)?;
    m.add_function(wrap_pyfunction!(score::cosine_similarity, m)?)?;
