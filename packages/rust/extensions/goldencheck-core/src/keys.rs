...
/// delegates to [`functional_dependency_holds_ids`].
...
#[cfg(test)]
fn tuple_distinct_count(columns: &[&[u64]], subset: &[usize]) -> u64 {
...
pub(crate) fn functional_dependency_holds_ids(lhs: &[u64], rhs: &[u64]) -> bool {
...
pub(crate) fn discover_functional_dependencies_ids(columns: &[&[u64]]) -> Vec<(usize, usize)> {
    ...
    for det in 0..n_cols {
        ...
        for dep in 0..n_cols {
            ...
            if functional_dependency_holds_ids(columns[det], columns[dep]) {
                out.push((det, dep));
            }
        }
    }
...
pub(crate) fn fd_violation_rows_ids(det: &[u64], dep: &[u64]) -> Vec<usize> {
    let modes = fd_group_modes(det, dep);
    det.iter()
        .zip(dep.iter())
        .enumerate()
...
pub(crate) fn discover_approximate_fds_ids(
    columns: &[&[u64]],
    min_confidence: f64,
...
pub(crate) fn composite_key_search_ids(
    columns: &[&[u64]],
    n_rows: usize,
    max_size: usize,
    single_unique: &[bool],
...
mod tests {
    ...
    fn fd_holds_and_breaks() {
        ...
        assert!(functional_dependency_holds_ids(&city, &country));
        assert!(!functional_dependency_holds_ids(&country, &city));
    }
    ...
    fn approximate_fd_surfaces_violations() {
        ...
        assert_eq!(fd_violation_rows_ids(&det, &dep), vec![5]);
        let cols: Vec<&[u64]> = vec![&det, &dep];
        let fds = discover_approximate_fds_ids(&cols, 0.8);
    ...
    fn approximate_fd_skips_near_unique_determinant() {
        ...
        let cols: Vec<&[u64]> = vec![&det, &dep];
        assert!(discover_approximate_fds_ids(&cols, 0.8)
            .iter()
            .all(|&(d, _, _)| d != 0));
    }
    ...
    fn discovers_fds_and_skips_trivial() {
        ...
        let fds = discover_functional_dependencies_ids(&cols);
    ...
    fn finds_minimal_composite_key() {
        ...
        let cols: Vec<&[u64]> = vec![&a, &b];
        let keys = composite_key_search_ids(&cols, 4, 3, &[false, false]);
    ...
    fn skips_unique_columns() {
        ...
        let cols: Vec<&[u64]> = vec![&a, &b];
        let keys = composite_key_search_ids(&cols, 4, 3, &[true, false]);
