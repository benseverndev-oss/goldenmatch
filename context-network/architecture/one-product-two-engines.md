...
Date: 2026-07-25
Status: **ACCEPTED** — this is the governing architecture frame. Recorded as decision
`context-network/decisions/0047-one-product-two-engines-architecture.md`; summarized in the
"governing arc" of `context-network/foundation/project-definition.md` and pointed to from the
always-loaded root `CLAUDE.md`. Amends the "every capability must reach every surface"
commitment (see §1.1) and extends the `docs/design/2026-07-01-rust-is-the-reference-roadmap.md`
model from "Rust is the reference kernel" to "one authoritative semantic owner per capability,
across two engines."

> This is a *foundation/architecture* node: it colors architectural decisions the way the
> North Star colors product decisions. When an architectural change competes with this frame,
> the change must either conform or amend this doc (+ decision 0047) in the same PR.
...
---
**Classification:** foundation/architecture • **Status:** accepted • **Last updated:** 2026-07-25
