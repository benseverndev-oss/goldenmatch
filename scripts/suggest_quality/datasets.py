...
_ORGS_HARD_DIR = Path(__file__).resolve().parent / "corpora" / "orgs_hard"


def _orgs_hard() -> tuple[pl.DataFrame, set] | None:
    """Curated hard B2B-organisation corpus. COMMITTED, so never skipped.

    Fills the panel's one structural gap: every other loadable dataset is a
    person with a name plus an address or an email, and every red on the
    benchmarks lane in one week was on a NON-person path. Generated by
    `scripts/suggest_quality/gen_orgs_hard.py`, which documents each hardness
    class; `test_orgs_hard_corpus.py` pins the committed bytes.

    The `hardness` column is dropped before matching -- it is a per-record
    label for diagnosing WHICH failure mode moved, never an input feature.
    """
    rec_p, truth_p = _ORGS_HARD_DIR / "records.csv", _ORGS_HARD_DIR / "truth.csv"
    if not (rec_p.exists() and truth_p.exists()):
        return None
    df = pl.read_csv(rec_p).drop("hardness")
    truth = pl.read_csv(truth_p)
    gt = {
        (int(a), int(b))
        for a, b in zip(truth["row_a"].to_list(), truth["row_b"].to_list())
    }
    return df, gt
...
REGISTRY: list[Dataset] = [
    ...
    # Curated, committed, deliberately hard. Non-person shape (see #2635 thread).
    Dataset("orgs_hard",           "anchor", _orgs_hard),
