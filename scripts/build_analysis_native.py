#!/usr/bin/env python3
"""Build the optional `goldenanalysis._native` acceleration extension.

Compiles the PyO3 crate at `packages/rust/extensions/analysis-native` (abi3)
against the *current* interpreter and drops the artifact at
`packages/python/goldenanalysis/goldenanalysis/_native.abi3.so` so `import
goldenanalysis._native` resolves.
...
Mirrors scripts/build_goldencheck_native.py (the goldencheck sibling).
"""
...
CRATE = REPO / "packages" / "rust" / "extensions" / "analysis-native"
PKG = REPO / "packages" / "python" / "goldenanalysis" / "goldenanalysis"
