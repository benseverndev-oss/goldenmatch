#!/usr/bin/env python3
"""Build the optional `infermap._native` acceleration extension.

Compiles the PyO3 crate at `packages/rust/extensions/infermap-native` (abi3)
against the *current* interpreter and drops the artifact at
`packages/python/infermap/infermap/_native.abi3.so` so `import infermap._native`
resolves.

Run via the project interpreter, e.g.:
    uv run python scripts/build_infermap_native.py
...
Mirrors scripts/build_analysis_native.py (the goldenanalysis sibling).
"""
...
CRATE = REPO / "packages" / "rust" / "extensions" / "infermap-native"
PKG = REPO / "packages" / "python" / "infermap" / "infermap"
...
def main() -> int:
    ...
    # `--offline` only when explicitly requested (e.g. a sandbox with a warm cargo
    # cache but no crates.io network). CI has network + a cold cache on first run,
    # so it must be allowed to fetch -- default is online.
